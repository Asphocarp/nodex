...
export type CodeBlockOptions = {
  ...
  /**
   * Resolves the default when a new code block is created. The schema default
   * remains deterministic while a product can retain a local language choice.
   */
...
export function getLanguageId(
  ...
  const normalizedLanguage = languageName.normalize("NFKC").trim().toLocaleLowerCase();
  return Object.entries(options.supportedLanguages ?? {}).find(
    ([id, { aliases }]) => {
      return (
        id.normalize("NFKC").trim().toLocaleLowerCase() === normalizedLanguage ||
        aliases?.some(
          (alias) => alias.normalize("NFKC").trim().toLocaleLowerCase() === normalizedLanguage,
        )
      );
    },
  )?.[0];
