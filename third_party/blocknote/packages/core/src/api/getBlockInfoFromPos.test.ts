...
import { getNearestBlockPos, getNodeId } from "./getBlockInfoFromPos.js";
...
describe("getNearestBlockPos", () => {
  it("resolves document-wide selection boundaries without warning", () => {
    ...
    try {
      const result = editor.transact((transaction) => ({
        first: getNearestBlockPos(transaction.doc, 0),
        last: getNearestBlockPos(transaction.doc, transaction.doc.content.size),
      }));

      expect(result.first.node.attrs.id).toBe("first");
      expect(result.last.node.attrs.id).toBe("last");
