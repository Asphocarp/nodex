...
type BlockContentKind = "inline" | "table" | "none" | "plain";
...
>(
  ...
  const blockNoteSchema = getBlockNoteSchema(pmSchema);
  const oldContentKind = blockNoteSchema.blockSchema[blockInfo.blockNoteType].content;
  const newContentKind = blockNoteSchema.blockSchema[block.type || blockInfo.blockNoteType].content;
  ...
  if (blockInfo.isBlockContainer && newNodeType.isInGroup("blockContent")) {
    ...
    tr.step(
      new ReplaceStep(
        ...
        new Slice(Fragment.from(replacementNode), 0, 0),
      ),
    );
...
>(
  ...
  oldContentKind: BlockContentKind,
  newContentKind: BlockContentKind,
  ...
  if (block.content) {
    ...
    if (oldContentKind === newContentKind && oldNodeType.spec.content === "") {
      // Keep the empty content when both Blocks share the same BlockNote
      // content model.
    } else if (
      oldContentKind === newContentKind &&
      newNodeType.spec.content === oldNodeType.spec.content
    ) {
      // keep old content, because the content type is the same and should be compatible
    } else if (oldContentKind === newContentKind && newNodeType.validContent(oldContent)) {
  ...
  if (content === "keep") {
    ...
    const replacementNode = newNodeType.createChecked(
      {
