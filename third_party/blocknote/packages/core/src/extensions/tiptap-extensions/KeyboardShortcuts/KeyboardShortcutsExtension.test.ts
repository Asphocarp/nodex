...
const atomicBlockSpec = createBlockSpec(
  {
    type: "atomicBlock",
    propSchema: {},
    content: "none",
...
describe("KeyboardShortcutsExtension atomic Backspace boundary", () => {
  it("does not treat a contentless atomic block as an empty text block", () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    const editor = BlockNoteEditor.create({
      schema,
      initialContent: [
        { id: "atomic-0", type: "atomicBlock" },
        { id: "paragraph-0", type: "paragraph", content: "after" },
      ],
    });
    editor.mount(document.createElement("div"));
    ...
    try {
      ...
      expect(editor.document.map(({ type }) => type)).toEqual(["atomicBlock", "paragraph"]);
      expect(
        warn.mock.calls.some(([message]) => String(message).includes("TextSelection endpoint")),
      ).toBe(false);
    } finally {
      warn.mockRestore();
      editor._tiptapEditor.destroy();
