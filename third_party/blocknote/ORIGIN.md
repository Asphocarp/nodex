...
- `react/src/components/Popovers/FloatingUIOptions.ts` and `react/src/components/Popovers/GenericPopover.tsx`: allow selection-reactive floating surfaces to opt into a static DOM snapshot during exit, while preserving the default live React subtree for drag-owned surfaces.
