...
import { readDatabaseModule } from "../src/main/local-store/database-module";
import { createProject } from "../src/main/local-store/projects";
import { DATABASE_MODULE_CONTRACT_VERSION } from "../src/shared/database-module";
...
const run = async (): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      const snapshot = readDatabaseModule(
        database, {
          version: DATABASE_MODULE_CONTRACT_VERSION,
          projectId: project.id,
          read: {
            target: { kind: "view", viewId: seed.id },
            mode: "query",
          },
        },
        {
      ...
      invariant(
        snapshot.value.value.kind === "query",
        "Database Module returned an incompatible read kind",
      );
      invariant(
        snapshot.value.value.value.view.name === seed.name,
      ...
      invariant(
        current.seq > snapshot.value.changeLogSeq,
