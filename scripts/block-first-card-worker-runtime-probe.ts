...
const readPersistedReferences = (
  cardId: string,
): readonly PersistedReference[] => {
  ...
  try {
    ...
    invariant(row, `Reference projection for ${cardId} is missing`);
    return JSON.parse(row.references_json) as readonly PersistedReference[];
  } finally {
    database.close();
  }
};

const readForeignReferenceMigrationStatus = (
  ...
  try {
    ...
    `,
      ...
      .get(sourceBlockId) as { readonly status: string } | undefined;
...
const readPersistedCardDocument = (cardId: string): PersistedCardDocument => {
...
const archiveCardForProbe = (cardId: string): void => {
  ...
  try {
    ...
    invariant(
      result.changes === 1,
      `Could not archive projection host ${cardId}`,
    );
  } finally {
    database.close();
  }
...
const corruptLatestPendingShadowFence = (cardId: string): void => {
  ...
  try {
    ...
    invariant(
      result.changes === 1,
      "Could not corrupt the pending shadow fence",
    );
  } finally {
    database.close();
  }
...
const setForeignMigrationFailureTrigger = (enabled: boolean): void => {
  const database = new Database(getDatabasePath(), { readonly: false });
  try {
    database.exec(
      "DROP TRIGGER IF EXISTS probe_reject_foreign_reference_migration",
    );
    if (!enabled) return;
    database.exec(`
      CREATE TRIGGER probe_reject_foreign_reference_migration
      BEFORE UPDATE ON foreign_reference_migrations
      BEGIN
        SELECT RAISE(ABORT, 'probe foreign-reference migration failure');
      END;
    `);
  } finally {
    database.close();
...
try {
  ...
  invariant(
    ...
    "Create ACK preceded Document readiness",
  );
  invariant(
    genesis.authority === "legacy_shadow",
    "Unexpected genesis authority",
  );
  ...
  invariant(
    genesis.projectedSeq === genesis.headSeq,
  ...
  invariant(
    afterTranslation.title === "Worker translated",
    "Translated title lagged",
  );
  invariant(
    afterTranslation.nfm === "Alpha\nBeta\nGamma",
    "Translated NFM lagged",
  );
  invariant(
    afterTranslation.ids.slice(0, genesis.ids.length).join(",") ===
      genesis.ids.join(","),
  ...
  invariant(
    ...
    "Restart fixture has no unrelated backlog",
  );

  const restartedWriter = new CardMutationWriter();
  const publicScopeBeforeMutation =
    await restartedWriter.getBlockDocumentProjectId(
      `document:${created.result.id}`,
    );
  invariant(
    publicScopeBeforeMutation.ok,
    "Public Document scope lookup failed",
  );
  invariant(
  ...
  invariant(
    recovered.title === "Recovered at restart",
    "Startup drain did not recover title",
  );
  invariant(
    recovered.nfm === "Alpha\nBeta\nGamma\nDelta",
  ...
  const afterFailure = await resilientWriter.createCard(
    project.id,
    "in_progress",
    {
  ...
  invariant(
    afterFailure.metrics.shadowJobsFailed === 1,
    "Failed shadow job was not observed",
  );
  invariant(
    countShadowJobs("failed") === 1,
    "Failed shadow job was not durable",
  );
  invariant(
  ...
  invariant(
    queuedResult.result.status === "updated",
    "Queued mutation failed during shutdown",
  );
  await shutdown;
  ...
  const directProjectionHost = await cutoverWriter.createCard(
    project.id,
    "draft",
    {
  ...
  const canonicalCardReferences = readPersistedReferences(
    directProjectionHost.result.id,
  );
  invariant(
    canonicalCardReferences.length === 1 &&
      canonicalCardReferences[0]?.kind === "block",
    "Card mutation ACK preceded canonical reference migration",
  );
  invariant(
    readForeignReferenceMigrationStatus(
      canonicalCardReferences[0]?.sourceBlockId ?? "",
    ) === "applied",
    "Canonical Card reference has no durable migration ledger",
  );
  archiveCardForProbe(directProjectionHost.result.id);
  const projectedTargetPreparation =
    await cutoverWriter.prepareOwnedBlockDocument(
      project.id,
      projectedTarget.result.id,
    );
  invariant(
    projectedTargetPreparation.ok,
    "Projected target preparation failed",
  );
  invariant(
    projectedTargetPreparation.value.authority === "ydoc_primary",
    "Canonical reference still blocked its target from Y.Doc authority",
  );
  const projectionHostPreparation =
    await cutoverWriter.prepareOwnedBlockDocument(
      project.id,
      directProjectionHost.result.id,
    );
  ...
  const queryHost = await cutoverWriter.createCard(project.id, "draft", {
  ...
  const canonicalDatabaseReferences = readPersistedReferences(
    queryHost.result.id,
  );
  invariant(
    canonicalDatabaseReferences.length === 1 &&
      canonicalDatabaseReferences[0]?.kind === "database_view",
    "Card mutation ACK preceded durable Database View migration",
  );
  ...
  invariant(
    queryRowPreparation.value.authority === "ydoc_primary",
    "Durable Database View still held possible rows on legacy authority",
  );

  enqueueLegacyUpdate(afterRestart.result.id, {
    description: `<card-ref project="${project.id}" card="${created.result.id}" />`,
  });
  setForeignMigrationFailureTrigger(true);
  const migrationBlockedCutover = await cutoverWriter.prepareOwnedBlockDocument(
    project.id,
    afterRestart.result.id,
  );
  invariant(
    !migrationBlockedCutover.ok &&
      migrationBlockedCutover.error.code === "document_not_ready",
    "A failed foreign-reference migration did not block cutover",
  );
  setForeignMigrationFailureTrigger(false);
  const migrationRetry = await cutoverWriter.prepareOwnedBlockDocument(
    project.id,
    afterRestart.result.id,
  );
  invariant(
    migrationRetry.ok && migrationRetry.value.authority === "ydoc_primary",
    "A resumable foreign-reference migration did not recover after retry",
  );
  const cutoverCandidate = await cutoverWriter.createCard(
    project.id,
    "in_review",
    {
  ...
  crossProjectPrepareRejected =
    !crossProjectPrepare.ok &&
  ...
  invariant(
    !applied.value.duplicate,
    "First collaborative update was duplicate",
  );
  invariant(
    documentBoardEvents.length === 1,
    "Document update did not publish one board event",
  );
  const documentBoardEvent = documentBoardEvents[0];
  invariant(
    documentBoardEvent?.cardId === cutoverCandidate.result.id,
    "Document event targeted the wrong Card",
  );
  invariant(
    documentBoardEvent.summary?.title === "Collaborative title",
    "Document event used the legacy Card title",
  );
  invariant(
    documentBoardEvent.summary?.descriptionPreview === "Primary body",
    "Document event omitted the materialized body preview",
  );
  invariant(
    documentEventObservedCommittedState,
    "Document event was published before its SQLite commit became visible",
  );
  ...
  const foreignTemplateRoot = openCardDocument(
    foreignTemplate.document,
  ).body.get(0);
  invariant(
    foreignTemplateRoot instanceof Y.XmlElement,
    "Foreign template root is invalid",
  );
  const foreignBlock = foreignTemplateRoot.get(0);
  invariant(
    foreignBlock instanceof Y.XmlElement,
    "Foreign template Block is missing",
  );
  const collaborativeRoot = collaborativeRoots.body.get(0);
  invariant(
    collaborativeRoot instanceof Y.XmlElement,
    "Collaborative body root is invalid",
  );
  const rejectedForeignUpdate = captureDocumentUpdate(
    collaborativeDocument,
    () =>
      collaborativeRoot.insert(collaborativeRoot.length, [
        cloneXmlSubtree(foreignBlock),
      ]),
  );
  ...
  invariant(
    !rejectedForeign.ok,
    "Primary Document accepted a legacy foreign-body projection",
  );
  invariant(
  ...
  const childlessSync = await cutoverWriter.syncBlockDocument({
    documentId: primaryDescriptor.value.documentId,
    clientSessionId: "childless-reference-probe",
    stateVector: new Uint8Array([0]),
  });
  invariant(
    childlessSync.ok,
    "Could not reload the primary Document after rejection",
  );
  const childlessDocument = new Y.Doc({
    guid: primaryDescriptor.value.documentId,
  });
  Y.applyUpdate(childlessDocument, childlessSync.value.update, "probe-sync");
  const childlessRoot = openCardDocument(childlessDocument).body.get(0);
  invariant(
    childlessRoot instanceof Y.XmlElement,
    "Childless probe root is invalid",
  );
  const canonicalTemplate = createCardDocumentGenesis({
    documentId: "document:canonical-reference-template",
    title: "",
    nfm: `<card-ref target-block="${created.result.id}" />`,
  });
  const paragraphTemplate = createCardDocumentGenesis({
    documentId: "document:canonical-reference-child-template",
    title: "",
    nfm: "Hidden reference child",
  });
  const canonicalTemplateRoot = openCardDocument(
    canonicalTemplate.document,
  ).body.get(0);
  const paragraphTemplateRoot = openCardDocument(
    paragraphTemplate.document,
  ).body.get(0);
  invariant(
    canonicalTemplateRoot instanceof Y.XmlElement,
    "Canonical template is invalid",
  );
  invariant(
    paragraphTemplateRoot instanceof Y.XmlElement,
    "Paragraph template is invalid",
  );
  const canonicalBlock = canonicalTemplateRoot.get(0);
  const paragraphBlock = paragraphTemplateRoot.get(0);
  invariant(
    canonicalBlock instanceof Y.XmlElement,
    "Canonical reference is missing",
  );
  invariant(
    paragraphBlock instanceof Y.XmlElement,
    "Reference child is missing",
  );
  const nestedGroup = new Y.XmlElement("blockGroup");
  nestedGroup.insert(0, [cloneXmlSubtree(paragraphBlock)]);
  canonicalBlock.insert(canonicalBlock.length, [nestedGroup]);
  const rejectedChildlessUpdate = captureDocumentUpdate(childlessDocument, () =>
    childlessRoot.insert(childlessRoot.length, [
      cloneXmlSubtree(canonicalBlock),
    ]),
  );
  const rejectedChildless = await cutoverWriter.applyBlockDocumentUpdate({
    documentId: primaryDescriptor.value.documentId,
    storeEpoch: childlessSync.value.storeEpoch,
    generation: childlessSync.value.generation,
    updateId: "probe:canonical-reference-child-rejected",
    clientSessionId: "childless-reference-probe",
    baseHeadSeq: childlessSync.value.headSeq,
    touchedBlockIds: [],
    update: rejectedChildlessUpdate,
  });
  invariant(
    !rejectedChildless.ok &&
      rejectedChildless.error.code === "invalid_document_update",
    "Primary Document accepted children under a canonical reference",
  );
  childlessDocument.destroy();
  canonicalTemplate.document.destroy();
  paragraphTemplate.document.destroy();
  ...
  invariant(
    ...
    "Idempotent Document retry was not marked duplicate",
  );
  invariant(
    documentBoardEvents.length === 1,
    "Idempotent retry published a duplicate board event",
  );
  collaborativeDocument.destroy();
  await cutoverWriter.shutdown();

  await initializeDatabase();
  const restartedPrimarySummary = readCardSummaryById(
    cutoverCandidate.result.id,
  );
  invariant(
  ...
  enqueueLegacyUpdate(directProjectionHost.result.id, {
  ...
  const publicScope = await shutdownOnlyWriter.getBlockDocumentProjectId(
    `document:${directProjectionHost.result.id}`,
  );
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      legacyReferencesMigratedBeforeMutationAck: true,
      canonicalReferencesNoLongerBlockedTargetAuthority: true,
      migrationFailureBlockedCutoverAndRetried: true,
      ...
      primaryWriterRejectedNestedCanonicalReference: true,
