...
export function browserRuntimeReleaseArguments(options: {
  ...
  for (const [architecture, filePath] of [["arm64", arm64Path], ["x64", x64Path]] as const) {
  ...
  return [
    ...
    "--verify-tag",
    "--latest=false",
    "--title", `Nodex Browser runtime ${options.tag.replace("browser-runtime-v", "")}`,
    "--notes", "Immutable Browser runtime closure consumed by Nodex release locks.",
  ];
}
...
export function publishBrowserRuntime(options: {
  ...
  const latestBefore = JSON.parse(run(["api", `repos/${options.repo}/releases/latest`])) as { readonly tag_name?: unknown };
  if (typeof latestBefore.tag_name !== "string" || !stableVersionFromAppTag(latestBefore.tag_name)) {
    throw new Error("Browser runtime publication requires Latest to already be a stable app release.");
  }
  run(browserRuntimeReleaseArguments(options));
  const latestAfter = JSON.parse(run(["api", `repos/${options.repo}/releases/latest`])) as { readonly tag_name?: unknown };
