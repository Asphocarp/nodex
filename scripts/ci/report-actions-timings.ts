...
const duration = (startedAt: string | null, completedAt: string | null): number => {
  ...
  return Math.max(
    0,
    parseTimestamp(completedAt, "completedAt") - parseTimestamp(startedAt, "startedAt"),
  );
};
...
export const timingRecordForRun = (run: ActionsRunRecord): RunTimingRecord => {
  ...
  const firstStartedAt =
    run.jobs
      .flatMap((job) =>
        job.startedAt ? [parseTimestamp(job.startedAt, `${job.name}.startedAt`)] : [],
      )
...
export const summarizeWorkflowRuns = (
  ...
  const runs = [...latestAttempts.values()]
    ...
    .sort(
      (left, right) =>
        parseTimestamp(right.createdAt, "run.createdAt") -
        parseTimestamp(left.createdAt, "run.createdAt"),
    )
  ...
  const successfulFirstAttempts = runs.filter(
...
export const parseReportArguments = (args: readonly string[]): CliArguments => {
  ...
  if (!workflow || !output || !Number.isSafeInteger(limit) || limit <= 0 || limit > 100) {
    throw new Error(
      "Usage: report-actions-timings --workflow <name> --limit <1-100> --output <path> [--repo <owner/name>] [--event <event>] [--branch <branch>].",
    );
  }
  return {
...
const main = (): void => {
  ...
  const report = summarizeWorkflowRuns(fetchRuns(repository, workflowId, options.limit, options), {
  ...
  writeFileSync(
    output,
    `${JSON.stringify({ repository, workflow: options.workflow, ...report }, null, 2)}\n`,
    "utf8",
  );
