...
export const focusBoardDenseUi = async (
  page: Page,
  manifest: ScenarioManifest,
): Promise<void> => {
  ...
  if (!primaryPageId) throw new Error("board/dense manifest has no primary Page");
  const primaryCard = page.locator(`[data-board-uuid-v7="${primaryPageId}"]`);
  await primaryCard.locator('[data-card-context-menu-trigger="true"]')
    .evaluate((element) => (element as HTMLElement).click());
...
export const observeBoardDenseUi = async (
  page: Page,
): Promise<BoardDenseUiObservation> => {
  const entries = await Promise.all(WORKFLOW_STATUS_ORDER.map(async (status) => [
    status,
    await page.locator(
  ...
  return {
    ...
    cardsByStatus,
    pageStageVisible: await page.locator('[data-page-stage-surface="true"]:visible')
      .isVisible(),
    editorVisible: await page.locator(
      '.nfm-editor .ProseMirror[contenteditable="true"]:visible',
    ).isVisible(),
  };
};
