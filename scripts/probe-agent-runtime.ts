...
async function probeAgentRuntimePromise(
  ...
  const first = await withProbeClient(
    callbacks,
    input.binaryPath,
    expectedStateHome,
    async (client) => {
      ...
      const providerEntries = readResultData(
        await client.request("interpreter/provider/list", { includeUnconfigured: true }),
      ...
      providers = await Promise.all(
        supportedProviderEntries.map(async (entry, index): Promise<ProviderSummary> => {
          ...
          const modelIds = summarizeModelIds(
            await client.request("interpreter/model/list", {
              modelProvider: id,
              includeHidden: false,
            }),
          );
          const harnesses = summarizeHarnesses(
            await client.request("interpreter/harness/list", {
              providerId: id,
      ...
      return { initialize, providers, modelHarnesses, firstResultCount };
    },
  );

  const restartedResultCount = await withProbeClient(
    callbacks,
    input.binaryPath,
    expectedStateHome,
    async (client) =>
  ...
  return {
    ...
    modelHarnesses: first.modelHarnesses,
    protocolSchemaSha256,
    providers: first.providers,
    runtimeVersion: lock.runtimeVersion,
    threadSearch: { firstResultCount: first.firstResultCount, restartedResultCount },
  };
}
