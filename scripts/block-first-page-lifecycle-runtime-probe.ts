...
} from "../src/main/local-store/page-lifecycle";
...
import { putProjectResourceGrantInDatabase } from "../src/main/local-store/project-resource-grants";
...
const request = (
  ...
  parsePageLifecycleMutationRequest({
    ...
    clientSessionId: "page-lifecycle-runtime",
...
const moveDatabasePageToLibrary = (
  scope: Scope,
  pageId: string,
  operationId: string,
): number => {
  ...
  const placement = database
    ...
    | undefined;
  invariant(placement, `Page ${pageId} is not Database-parented`);
  const result = applyBlockTransfer(database, {
    ...
    mode: "move",
    rootBlockIds: [pageId],
    expectedLocationRevisions: { [pageId]: placement.location_revision },
    source: {
      kind: "database",
  ...
  const parentRevision = result.value.finalLocationRevisions[pageId];
  invariant(
    parentRevision !== undefined,
    `BlockTransfer omitted Page ${pageId} location revision`,
  );
...
const main = async (): Promise<void> => {
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), "nodex-page-lifecycle-runtime-"),
  );
  process.env.NODEX_DIR = directory;
  try {
    await initializeDatabase();
    const project = createProject({ name: "Page lifecycle runtime" });
    ...
    const created = commit(
      scope,
      "runtime:create-card",
      createOperation(pageId, "Block authority"),
    );
    ...
    const boardProjection = readDatabasePageById(getDb(), project.id, pageId);
    invariant(
      boardProjection?.title === "Block authority" &&
        boardProjection.description === "Durable body paragraph",
      "Page read model did not assemble from Block/Document authority",
    );
    ...
    closeDatabase();
    await initializeDatabase();
    const createRetry = applyPageLifecycleMutation(
      getDb(),
      request(
        scope,
        "runtime:create-card",
    ...
    const libraryPageId = createUuidV7();
    commit(
      scope,
      "runtime:create-space-card",
      createOperation(libraryPageId, "Space Page"),
    );
    const libraryParentRevision = moveDatabasePageToLibrary(
      scope,
      libraryPageId,
      "runtime:card-to-space",
    );
    moveDatabasePageToLibrary(scope, anchorId, "runtime:anchor-to-library");

    const moved = commit(scope, "runtime:move-card", {
      kind: "move_page_in_library",
      pageId: libraryPageId,
      expectedParentRevision: libraryParentRevision,
      beforeBlockId: anchorId,
    });
    invariant(
      moved.parentRevision === libraryParentRevision + 1,
      "Space Page reorder did not advance its location revision",
    );
    ...
    invariant(
      ...
      "Deleted Page retained a top-level placement",
    );
    ...
    const injected = applyPageLifecycleMutation(getDb(), injectedRequest);
    invariant(
    ...
    closeDatabase();
    await initializeDatabase();
    const injectedRetry = applyPageLifecycleMutation(getDb(), injectedRequest);
    invariant(
      !injectedRetry.ok &&
    ...
    const continuityAfter = verifyPageDocumentContinuity(
      getDb(),
      project.id,
      pageId,
    );
    ...
    const standaloneDeleted = commit(scope, "runtime:delete-standalone", {
      ...
      expectedParentRevision: standaloneState.location_revision,
    });
    const standaloneRestored = commit(scope, "runtime:restore-standalone", {
      ...
      expectedParentRevision: standaloneDeleted.parentRevision,
      membership: null,
    });
    invariant(
      standaloneRestored.membershipId === null &&
        standaloneRestored.documentId === standalone.documentId,
      "Standalone Page restore manufactured membership or Document identity",
    );
    ...
    getDb()
      ...
      .run(project.id);
    const invalidOption = applyPageLifecycleMutation(
      getDb(),
    ...
    const faultPoints: readonly PageLifecycleMutationFaultPoint[] = [
      "after_identity",
    ...
    closeDatabase();
    await initializeDatabase();
    const lostResponseRetry = applyPageLifecycleMutation(
      getDb(),
      request(
        scope,
        "runtime:create-lost-response",
    ...
    const epochMismatch = applyPageLifecycleMutation(
      getDb(),
      parsePageLifecycleMutationRequest({
        ...request(
          scope,
