...
- `lib/kanban-store.ts`: shared per-Project `BoardSummarySnapshot` store with Project/Database/Data Source/View identity, Store epoch/change-log cursor, deduped summary fetches, optimistic journal rebase (`baseBoard + pending/acknowledged/local ops`), semantic convergence collection, LWW conflict superseding, typed conflict resolution (`updated|conflict|not_found`), and O(1) compatibility `pageIndex` lookup. A successful command ends pending UI while its acknowledged transform remains visible until exact canonical base satisfies the same identity/order/field intent. Durable projection impact owns freshness; singleton row effects preserve Core position evidence and grouped windows remain scope-pure. Cursor-fenced `board-changed` summaries remain only provisional low-latency patches.
