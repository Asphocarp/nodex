...
- `block-transfer-ipc.ts` and `block-transfer-http.ts`: equivalent trusted Adapters for logical same-Library parent Move/Copy. They replace caller audit/scope identity, evaluate Project access to source and target, and return binary Document commits natively over IPC or as bounded base64 over HTTP. Desktop IPC selects the native Library Block-transfer Adapter under Rust authority for ordinary Page/Document subtrees, ordinary-root promotion/wrapping into Library or Data Source Pages, existing Page moves across Page Documents, Library, and Data Sources, and recursive Page ownership copies into Library, Data Source, or root Page placements. Existing Page moves reuse the Database membership/value/View-position kernel, update only the containing Page shell Documents, and never rewrite the moved Page's owned Document. Recursive Page Copy fences every source-owned Document, assigns deterministic fresh identities to the complete ownership closure, emits Yjs genesis commits for new Documents, and retains read-grant semantics while rehoming copies under the destination Project. The loopback HTTP path remains on its existing boundary until its Core client slice is enabled.
