...
A Database is a Container Block that owns Data Sources and Views. A Data Source owns its schema, row Pages, values, and query identity. A View targets exactly one Data Source and owns presentation configuration and Page placement for that View. These identities are independent and must not be derived from one another.
...
6. Every active Page has one exclusive acyclic parent. References, Views, relations, mentions, and backlinks are non-owning and non-authorizing.
