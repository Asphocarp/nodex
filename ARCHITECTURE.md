...
the only panel placement and ordering source. A Project owner uses the current
...
Project Scenes additionally persist an Agent Dock visibility, exact Session or
unbound-draft binding, and draft identity; Session Scenes have no Agent Dock.
...
The Window Session catalog persists complete Workbench layout v5
...
- `components/workbench/workbench-runtime.tsx`: the selected Workbench integration Module. It composes Window State, the Session Catalog, owner-scoped Scene materialization, the Panel Controller, Sidebar, Chrome, route, command palette, and selected Project/Session hosts without becoming the authority for Query, Codex, Browser, Terminal, or editor data.
...
10. The Project Agent Dock is a footer-only view of a normal Project Session and its canonical Codex conversation. Selecting a Chat never navigates or captures Browser ownership. `New chat` creates no Session until first send; creation, binding, and Thread start advance forward so a failed start retains one retryable blank Session. Dock binding and visibility persist without entering Back/Forward history, and hiding the Dock removes only its presentation claim and composer reserve.
