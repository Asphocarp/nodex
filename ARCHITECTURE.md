...
fresh restricted `.generated` Profile and reports the reopened v87 Store's
...
occurrence surface. Store Administration owns v87 readiness, backup listing,
...
semantically validates the complete v87 Document/Canvas/projection/managed-asset
...
1. Core accepts the exact published v26, v57, v68, v82, and v83 TypeScript inventories as historical import sources, the exact final TypeScript v84 inventory as its direct handoff, and exact Rust-owned v85/v86/v87 stores. For a historical source, Core identifies the complete normalized physical inventory, takes an online database snapshot plus a validated asset-tree backup, and invokes the bundled hash-pinned migrator only inside a staging Profile until it reaches exact v84. Core then reconstructs authoritative Yjs content through Yrs, rebuilds only derived projections, validates the complete v84 handoff, and atomically publishes Rust ownership as v87 under a crash-recovery journal. Rust-owned v85 and v86 stores are validated exactly, backed up, and atomically upgraded before v87 publication. The v87 forward migration widens null tab ownership to Browser, Terminal, and exact-file Files while removing legacy Terminal config Project fields. Unpublished same-version lineages, near-matches, ambiguous owners, and future stores fail closed; a Rust-owned v87 store is likewise validated exactly and is never silently repaired.
