...
6. Renderer shared project stores (`kanban-store`) receive IPC/SSE board-change signals and dedupe refresh work per project. Workbench session lists receive IPC/SSE session-change signals and reload the affected project sessions. The shared project list store receives `projects-changed` over IPC or `/api/projects/events` and refreshes project ordering and pinned state.
