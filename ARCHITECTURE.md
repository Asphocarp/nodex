...
panel, tab, and Thread-link routes dereference that same Workspace port; one
shared TypeScript-oracle port supplies the legacy implementation for both IPC
and HTTP instead of duplicating store mappings. Owned Document descriptor reads
...
through the existing strict renderer contracts before IPC. Semantic Agent
...
Core-signed event-head cursors. Remaining deep Codex persistence adapters stay on
...
ledger coordinate inside Core. Electron IPC and the loopback route dependency
select this same authority. The Hono reference, Library/Database Module, Page
detail/lifecycle/history, semantic Document command/history, and Block Transfer
routes now dereference the same process-selected desktop bridges as IPC. Their
TypeScript fallbacks preserve the caller's distinct `http_loopback` access and
audit identity rather than impersonating an application window. Realtime
The binary Yjs/Canvas realtime plane, descriptor/prepare calls, and SSE
subscriptions also select the same async Document bridge; subscription
authorization completes before Hono publishes a successful event stream.
The remaining Project/settings/assets/Codex store-shaped routes still gate
starting the whole Hono server under Rust.
...
- `http-server.ts`: Hono routes for Profiles/Libraries, Projects and bindings/grants, project sessions/tabs/thread links, Pages, Database/Source/View reads and writes, history, backups, assets, and access-bound Owned Document preparation/synchronization/mutation/version checkpoints. Yjs sync uses bounded binary envelopes; Canvas scene sync uses bounded canonical JSON; stable-ID Agent mutations and immutable Document-history commands use bounded JSON with host-bound identity. Realtime delivery is filtered through the Project's effective Library scope while Yjs Awareness remains ephemeral. Preparing a Page Document is an explicit writer command, never a renderer ID convention. Project/Session/panel/tab/Thread-link operations, Board/row/search projections, Calendar occurrence commands, Backup administration, reference reads, Project/trusted-Library Property and Database operations, Library operations, Page detail/lifecycle/history, semantic Document commands/history, Block Transfer, and the Project/Library binary Yjs/Canvas realtime plane dereference authority-selected desktop ports at request time. Backup restore schedules the same controlled host relaunch as IPC, while backup settings restart the authority-selected scheduler rather than the retired TypeScript scheduler. Project SSE reauthorizes Page events through the selected reference-read port. Arbitrary SQL inspection is a TypeScript-oracle development surface and returns 404 under native authority rather than proxying private Store access. SSE authorization completes before the route returns success, and loopback Library access keeps its `http_loopback` identity through the TypeScript fallback. Settings and assets remain host-owned filesystem concerns. Electron starts this same loopback server after either authority reports ready; native mode serves every registered data route through the selected deep-Module ports and never opens the Profile database in the Host.
...
- `block-transfer-coordinator.ts`, `document-sync-hub.ts`, `document-sync-runtime.ts`, and `document-sync-http.ts`: one targeted realtime plane shared by browser and desktop clients, plus one authority-neutral Block transfer lease algorithm. Hono adapts browser targets to the same async desktop Document port used by IPC; the TypeScript selection delegates to the Hub, while Rust subscribes directly to Core and never opens the TypeScript writer. The realtime plane requires subscribe-before-sync, evaluates current Project resource access, fans out durable engine-discriminated updates, keeps Yjs Awareness ephemeral, and repairs Canvas gaps with canonical full-scene sync. `BlockTransfer`, Additional Document commands, and identity-destructive Agent operations reuse one short per-surface flush/freeze lease. The Rust desktop bridge registers its native Yjs/Canvas subscriptions with the same pure lease coordinator and owns restore plus Block-transfer preparation/release without invoking the TypeScript Hub. A typed exact fence binds the complete Document-head closure, root location revisions, and source Data Source memberships; Library/Data Source Page moves therefore remain race-safe even when no collaborative Document needs freezing. Editability changes in place without remounting parent EditorViews/nested participants; an accepted provider lease may drain headlessly past visual teardown. Post-commit update or resync dispatches through the registered engine.
- `block-transfer-ipc.ts` and `block-transfer-http.ts`: equivalent trusted Adapters for logical same-Library parent Move/Copy. They replace caller audit/scope identity, evaluate Project access to source and target, and return binary Document commits natively over IPC or as bounded base64 over HTTP. Desktop IPC and loopback HTTP select the same authority-aware Document/Library bridge under Rust authority for ordinary Page/Document subtrees, ordinary-root promotion/wrapping into Library or Data Source Pages, existing Page moves across Page Documents, Library, and Data Sources, and recursive Page ownership copies into Library, Data Source, or Page placements, including nested and multi-root target insertions. Existing Page moves reuse the Database membership/value/View-position kernel, update only the containing Page shell Documents, and never rewrite the moved Page's owned Document. Recursive Page Copy fences every source-owned Document, assigns deterministic fresh identities to the complete ownership closure, emits Yjs genesis commits for new Documents, and retains read-grant semantics while rehoming copies under the destination Project. Multiple copied roots stage their independent ownership closures first and then enter a target Page through one ordered Yjs structure commit.
- `additional-document-command-ipc.ts` and `additional-document-command-http.ts`: equivalent public boundaries for Additional Document commands. They reject unavailable capabilities, validate route scope and bounded JSON, replace caller attribution with trusted host identity, and preserve typed outcomes. Desktop IPC and loopback HTTP select the same Owned Document Module bridge under the chosen authority.
...
- `codex/codex-service.ts`, `nodex-agent-authority-port.ts`, and the TypeScript/Core authority Adapters: domain facade for account/auth, thread/turn actions, sidebar thread sync/snapshot/pin APIs, external thread session materialization, background-terminal controls, side-chat forks, approval and user-input handling, packaged runtime resolution, canonical conversation state, state-owner routing, authorization-presentation routing, and shell/control events. New eligible Project-bound root tasks start with the static ten-tool `nodex_app@5` catalog. Main persists Nodex's selected permission preset separately from raw Codex config, verifies both before granting built-in Full access, then begins and captures exact Turn authority through one backend-selected asynchronous port. Under the native selector, Core derives and durably freezes Project or Library provenance before a dynamic tool executes; Electron never opens SQLite for the freeze, capture, or post-consent freshness checks. The authority is atomically bound to the first trusted exact Turn ID; app-server background child Turns may inherit only the exact parent Library authority captured at spawn, while independent created tasks do not. Electron continues to own Codex app-server execution, notification ordering, and consent presentation. The durable catalog selects execution and existing tasks are never mutated on resume.
...
- `agent-tools/dynamic-service-v3.ts`, `agent-tools/authorization-broker.ts`, `nodex-agent-resource-authority-port.ts`, `core-client/desktop-nodex-agent-dynamic-service.ts`, the native Library Agent-authorization compiler, and `codex/nodex-agent-dynamic-tool-runtime.ts`: unified resource-authority and consent boundary for all ten `nodex_app@5` tools. Every Nodex call executes directly in main with frozen Turn authority and never detours through the canonical conversation-state owner. Process startup installs one authority-selected semantic service before windows open, so native mode cannot fall through to the TypeScript writer or Document Hub; unported semantic tools fail closed. Native `get_context` composes Workspace Project and Database catalog snapshots, including inactive/archived write availability and the public Markdown guide, without opening SQLite. Native `fetch` resolves Page and ordinary Block targets together with the owning Page projection through one authorized Library snapshot, then reads one exact owned-Document snapshot with canonical content, requested intrinsic/Data Source properties, optional membership, narrow title/body/Block ETags, and Core-signed stale-detecting pagination. Native `search` builds one bounded Library candidate set, applies exact Turn overlays plus primary-Database/recursive-grant authority before ranking and slicing, combines fuzzy Page metadata/property evidence with fresh title/body FTS, and uses signed event-head cursors whose semantic identity excludes per-call authorization while every continuation reauthorizes. Native `query_database_view` and `query_data_source` execute saved or ad-hoc filters/sorts inside Database authority, revalidate exact Agent access there, slice the coherent query projection before it crosses UDS, and use Core-signed event-head-bound cursors. Native `update_page` and `advanced_update_page` prepare and reprepare title, exact-patch, guarded replacement, structural insertion, and ordered stable-Block insert/update/move/delete commands against current Page/Document authority; they preserve caller match cardinality, execute with a Core-minted single-use token, and project exact committed effects plus durable semantic ETags and deterministic local Block identities. Core classifies deleted Page owners in the exact footprint and requires an explicitly bound `allowDeletingOwnedBlocks` intent before preparing or executing them. A separate backend-selected asynchronous port plans resource intents and persists Project consent. Under the native selector, Library Core first validates the exact immutable Turn provenance, resolves Page/Block/Database/Data Source/View ownership plus recursive grants in one read snapshot, and classifies each intent as direct, consent-eligible, or denied. Persistable Page/Database consent commits as one canonical batch with one immutable receipt/event; Electron never opens SQLite for either side of the consent card. The primary Database and recursive `read_write` grants execute directly; `read` grants require consent only for writes; known ungranted same-Library targets require resource-scoped consent. One-call overlays bind exact Thread/Turn/call identity, task overlays bind root task/Project/Library/store epoch and survive renderer replacement, and Library-scope Turns auto-approve without a card. Planning alone is never execution authority: every owning Module revalidates the persisted Turn and exact overlay against current resource coordinates in its read or preparation transaction, and write Modules repeat that check in the committing writer transaction before consuming the prepared token. Writes in every path perform mutation-free prepare/reprepare, compare exact semantic and ownership footprints, and execute through the selected Module authority. Turn/provenance-bound receipts replay exact committed calls without prompting or mutating again, while missing prepared provenance and same-call/different-input collisions reject.
