...
Database authority at read time. Session reads
contain only Project or projectless scope, the durable database-starter marker,
...
same supervised bridge and exact-target lifecycle but remain Project-only. Canvas and Yjs
...
bridge. The Adapter submits one deep owner command to the exact Project
client and maps the native mutation effect, event sequence, and persisted
commit time back into the established receipt. Renewable Document heads and connection
...
identity-scoped Page/detail and owned-Document paths. Page creation, ordinary Block-to-Page promotion,
...
routes now dereference the same Core-backed desktop bridges as IPC and preserve
the caller's distinct `http_loopback` access and audit identity rather than
impersonating an application window. The binary Yjs/Canvas realtime plane,
descriptor/prepare calls, and SSE
...
- `codex/codex-app-server-client.ts`: global JSON-RPC client for the pinned Open Interpreter `app-server` stdio lifecycle, handshake/home verification, request correlation, reconnect/backoff, and wire-level typing against the committed `@nodex/codex-app-server-protocol` workspace package. The packaged runtime is release-lock and manifest validated; production never selects an arbitrary binary from `PATH`.
...
- `core-client/desktop-project-workspace-bridge.ts`: persistence adapter for canonical Codex Thread metadata, global sidebar pin/order state, session ownership, and parent-linked subagent rows. All reads and mutations are typed Workspace Module requests.
...
- `bin/nodex.mjs`: the npm-bin compatibility launcher. It validates a regular non-symlinked native `nodex` executable and forwards arguments without a shell; it implements no commands, HTTP client, SQL access, or storage fallback.
...
- `lib/electron-document-sync-adapter.ts` and `http-document-sync-adapter.ts`: equivalent provider transports. Electron uses typed binary structured-clone IPC. Exact local sessions are multiplexed by subscriber instance, and subscribe/unsubscribe reconciliation is serialized so an old disposer cannot close a revived session. Browser opens its Project- or Library-scoped SSE subscription before issuing versioned binary POST commands; EventSource reconnect triggers a new state-vector repair rather than trusting event delivery as causal authority. A Library descriptor is validated without a Project coordinate before the renderer creates its local editor-surface scope.
...
Board read flow:
1. All primary and non-primary board consumers use `database:view-window:get` or `GET /api/projects/:projectId/database-views/:viewId/rows`. Core applies the View filter/order in SQLite, reads `LIMIT first + 1`, and returns a signed keyset continuation. One effective-group SQL expression (explicit position group first, else the normalized grouping Property value, with NULL/empty-string/empty-list all meaning unassigned) drives the summary projection, the effective-group-major total order, group scoping, and group totals, so a grouped View also serves per-group windows (`groupScope`) and a bounded `database:view-groups:get` totals read whose counts always agree with scoped traversal. Each window is independently bounded below the ordinary transport ceiling and contains no Page body. These channels return a typed `CoreReadResult` envelope so cursor rejections and retryable failures reach the renderer as codes, not flattened strings; the HTTP routes map the same codes onto 400/403/404/409 responses.
2. Page Stage reads versioned `PageDetail` through `pages:detail:get` / `GET /api/projects/:projectId/pages/:pageId`, assembled in one SQLite read transaction after Project grant evaluation. The contract contains Page identity/parent, the owned Document exact-head projection, intrinsic properties, and—only for a Source-parented Page—the matching membership, Database Container, Data Source schema slice, and values. Library- and Page-parented Pages expose the same core Stage without fabricated Source fields. Database row detail is an identity read; list consumers never batch-hydrate bodies.
...
9. Project creation seeds one ordinary pinned `Database View` Session carrying the durable `database_starter` marker. A Window Session with no existing view record for a database-starter Session materializes one full-width local right-panel tab targeting the Project's current primary Database default View; the View identity is resolved from the Project read model at materialization time and is never stored on the Session. Closing every local tab consumes no Core state and does not cause reseeding.
10. Local `db_view` descriptors are unique per durable `databaseViewId` within one Session view, while `review` remains the singleton right-panel kind. Browser descriptors use Window Session-scoped Browser runtime identities. Terminal descriptors carry a PTY resource ID and acquire one interactive lease at a time. Files, Page Stage, Review, and Database View descriptors retain explicit target authorization context where required.
11. `WorkbenchSessionViewSnapshot` is the strict tagged tab contract. Descriptors carry stable resource targets but no Session owner, panel, or order; the containing view and its two panel trees provide those coordinates. Malformed kind/config combinations fail at the Window Session codec boundary.
12. Panel previews remain renderer-memory state. Files and Browser previews occupy one preview slot per Session panel leaf, replace each other within that leaf, and enter the Window Session snapshot only when pinned.
13. Renderer-local side-chat tabs are also outside SQLite but use a separate leaf-scoped lifecycle from previews. The renderer creates `sidechat-loading:<parentThreadId>:<index>` tabs, asks main to start an ephemeral fork, replaces the loading tab with `sidechat:<threadId>`, and discards the backing temporary thread when the tab closes.
...
- Arbitrary SQL inspection is not exposed through IPC, loopback HTTP, or the
