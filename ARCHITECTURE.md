...
`(store_epoch, commit_seq)` coordinate. Manifest verification loads the parent,
physical evidence, and canonical Document effects once as one verified bundle;
authorization and delivery reuse that bundle instead of rescanning epoch-wide
child tables. Raw physical payload bytes are checked against their persisted
effect digest before typed reconstruction. Compacted Document update bytes are
not historical authority: retained update receipts supply the immutable base
head, result head, digest, and byte length needed by the Manifest. The Store
epoch is therefore part of both semantic identity and the physical lookup
contract, rather than a foreign-key decoration.

Store preparation validates LocalCommit coverage and finalization once before
the runtime starts. Authorized stream reads then use the current Store epoch's
indexed first/last coordinates and keyset pages; they do not rescan the complete
ledger to validate every page. The exclusive Core writer and atomic mutation
transaction preserve that invariant until the next cold-open validation.
...
Durable user mutations have one immutable semantic Manifest. Core assigns its
`(store_epoch, commit_seq, manifest_hash)` identity in the SQLite transaction
...
the parent identity, ordered physical evidence digest, semantic index,
projection effects, routing claims, receipt ref, and Document reference
metadata. It deliberately excludes inline bytes and recipients. A ref-only
packet cannot encode `document_updated` bytes as a Module payload; those bytes
cross the boundary only through an authorized Document resource. If replay finds a verified
...
Renderer stores order each derived read model by Core's exact projection
coordinate: `(store_epoch, canonical_scope_key, schema_version, revision,
covered_commit_seq, effect_hash)`. A global LocalCommit sequence is durable
stream progress, never a projection version. Core compiles one exact effect per
...
candidate timing and bounded, monotonic migration progress only after Core has
authoritatively classified a supported older schema and before its first
migration write; they cannot select a process, extend authority, or replace the
terminal selection result and authenticated generation handshake. Historical
LocalCommit artifacts are rebuilt once against the final target schema in the
same atomic upgrade, rather than once per intermediate schema version. The
validated source backup is published under its content digest; a source with a
complete append-only LocalCommit source identity can reuse that exact recovery
artifact before copying again, while older sources without such an identity
still converge on one content-addressed file. A low-rate migration heartbeat
covers backup, integrity checking, and other phases without a countable unit.
Native CLI launch does not opt
...
receives the true migration state and changes from migration to workspace
opening as soon as Core reports Store readiness.

The launcher owns a candidate until authenticated connection succeeds. Any
valid startup frame resets a sliding inactivity deadline, while an independent
hard deadline remains finite; the authenticated connection gets a fresh
deadline after selection. Malformed output, cancellation, inactivity, artifact
mismatch, or connection failure terminates and awaits the entire candidate
process group before returning failure. Shutdown aborts and awaits candidate
selection or recovery already in flight. Only a connected detached Core is
unreferenced and allowed to outlive Electron. Main initialization
routes every durable port through Core before renderer windows become
operational; disconnecting Electron from an established authority does not
terminate the detached Core. The active
...
than assembling mutable coordinates in Electron. The bounded preflight returns
the default Database/View descriptor, exact tags Property revision, reserved
Block identity, Page/Document authority, parent and Library placement,
Data Source membership/status/View position, and lifecycle revisions; it never
returns an unbounded row list. Create carries explicit `start | end | before`
View-placement intent, and Core resolves that intent against the target group's
complete logical order inside the same writer transaction that inserts the
Page. When that order still contains unpositioned active Pages, the same rank
plan materializes their positions before inserting the new Page. The strict Adapter
maps the native snapshot into the lifecycle-v2 compiler contract, and Electron
IPC selects it through the authority bridge.
...
Renderer intent classification is governed by the closed owner-operation
matrix in the product spec and `typed-owner-blocks.ts`. Generic Document
materialization remains a fail-closed second boundary: removing a live Page,
Database, or Canvas owner returns `protected_owner_deletion`, while `pageRef`
remains a non-owning ordinary Block. This error is a programming/unsupported-
operation signal, not the normal nested-Page deletion workflow.
...
MCP Apps use a separate Main-owned sandbox aggregate modeled after the desktop
host's other untrusted guest runtimes. A trusted top-level renderer may request
one stable DNS-safe Skybridge source and non-persistent partition derived from
the MCP server or Codex App identity. The renderer sets the partition before it
mounts the webview, installs the handshake listener, and only then assigns the
source URL with a random init id in its hash. During `will-attach-webview`, Main
validates that source/partition/init tuple, registers a 30-second pending
attachment against the exact owner and Electron Session, and replaces renderer
preferences with the fixed guest preload, sandboxing, context isolation, no
Node, no nested webviews, and no insecure content. `did-attach-webview` consumes
that pending record by Session, owner, and init id; when Electron has not yet
published the guest URL, it consumes the first pending record for the same
Session and owner, matching Electron's attachment order. No claim IPC or
Electron-internal webview instance id participates in MCP App authority. A
process-wide guest registry forwards
the real Skybridge `{ ports, replyPort }` initialization as enumerated
MessagePorts, after which business RPC runs directly over those ports. The
renderer runtime owns presentation only: one capability-keyed webview is
reparented between inline, side-panel, and fullscreen surfaces without reload,
while a fixed thread/server scope constrains tool and resource proxy calls.
Widget follow-ups re-enter the owning thread action; external opens accept
bounded credential-free HTTPS URLs only.

The app renderer itself is not an untrusted guest. Production loads only from
the privileged read-only `app://-/index.html` origin with a strict response
CSP; development uses the exact configured Vite origin and the equivalent
policy. Main accepts
privileged renderer IPC only from an owned top-level window at that exact
production or development origin. BrowserWindow sandboxing remains enabled and
the preload contains no Node imports; filesystem, clipboard, and local-path
inspection stay in Main behind typed bounded channels.
...
- `app-renderer-policy.ts`, `clipboard-paste.ts`, and `mcp-app/*`: the exact top-level renderer CSP, bounded native-paste transport, and transport-neutral MCP resource/scope/sandbox identity contracts. MCP scope fixes one thread and server, then admits only the current/declared tools and exact declared resources; `openai/fileParams` cannot expand it.
...
- `bootstrap.ts`: early Electron lifecycle entrypoint. It resolves the Nodex home, publishes the canonical absolute path as `NODEX_HOME`, scopes `userData`/`sessionData`, registers the privileged `app` and MCP sandbox schemes before Electron readiness, owns the profile-scoped single-instance lock and deep-link queue, runs the packaged macOS Applications prompt, and dynamically imports the application runtime. The application runtime later installs the read-only `app://-` renderer handler on the ready default session.
- `main-runtime.ts`: application runtime startup (Core readiness gating, Core-backed adapter construction, read-only app/managed-asset protocol lifecycle, multi-window registry, per-window MCP App sandbox host, app-update service, event fanout, renderer permission policy, and shutdown handlers). The app session allows clipboard writes and managed raster delivery only to owned app windows; MCP and Browser guest partitions and embedded/subframe content do not inherit those capabilities.
- `app-renderer-protocol.ts` and `mcp-app/*`: read-only production app delivery plus the MCP guest attachment, per-session protocol/cache, permission, navigation, popup, download, request, MessagePort, and lifecycle host. Renderer-authored web preferences never survive the attachment boundary.
- `clipboard-paste-inspector.ts`: Main-owned native clipboard inspection. The synchronous paste-event path returns only bounded format/path metadata; rich text/image payload transfer is separately bounded and asynchronous, and path candidates must be absolute non-symlink filesystem entries.
...
- `index.ts`: sandbox-safe `window.api` bridge with no Node imports. It exposes typed invoke/event surfaces, bounded synchronous metadata-only paste/asset inspection, asynchronous paste payload reads, the focused-window Workbench command ingress, native context menus, and the owner-side MCP MessagePort bridge.
- `mcp-app-sandbox-guest.ts` and the owner handoff in `index.ts`: the fixed guest preload and trusted owner port delivery. The guest accepts the real Skybridge `{ ports, replyPort }` initialization, exports no Electron or Node capability to widget code, and forwards only the exact enumerated ports through Main to the current non-null owner origin and initialization id.
...
- `lib/mcp-app/*` and `features/local-conversation/view/shared/tools/mcp-capability-view-frame.tsx`: the renderer presentation Adapter for an already-authorized MCP App. It normalizes resources, creates one fixed-scope RPC dispatcher and one capability-keyed runtime, performs the owner/guest MessagePort handshake, forwards bounded host context/theme/tool updates, and reparents the single guest between presentation surfaces. It is not attachment, network, permission, or resource authority.
...
- `lib/authority-freshness-index.ts` and `lib/resource-authority-query-cache.tsx`: bounded renderer admission for Core-stamped canonical reads. Request leases contain only address/subject/request dependencies; same-snapshot response stamps supply dynamic authorization roots and a covered commit floor. Exact visibility changes fence matching registrations, while conservative or address reset fences the whole address. Root-floor GC is blocked by active registrations and older in-flight reads; overflow fails closed.
- `lib/kanban-store.ts`: shared per-Project-and-explicit-View `BoardSummarySnapshot` store with Project/Database/Data Source/View identity, Store epoch/change-log cursor, deduped summary fetches, optimistic journal rebase (`baseBoard + pending/committed/local ops`), receipt-fenced convergence collection, LWW conflict superseding, typed conflict resolution (`updated|conflict|not_found`), structurally shared snapshots, and O(1) compatibility `pageIndex` lookup. A successful command ends pending UI without promoting its detail result into canonical View state. The committed transform remains visible until a canonical read in the same Store epoch covers the full receipt cursor **and** its bounded window materializes the affected Page (or proves removal); a provisional projection patch is never materialization evidence. This prevents a tail create/move from disappearing merely because it lies outside the first loaded window. A Store-epoch replacement atomically fences the old journal, projection runtime, cursor floors, subscription, and detail authority; late callbacks are marked superseded and cannot repopulate the new cache. Durable projection impact owns freshness. A singleton row patch is emitted only when Core can prove exact active membership and manual order for the primary View; filtered, independently sorted, unpositioned, archived, and multi-row changes are patchless floor reads. Exact row reducers rebuild the affected bounded window from rank coordinates rather than mixing one row's ordinal with stale sibling ordinals, admit new rows only inside the loaded span, and keep grouped windows scope-pure. Cursor-fenced `board-changed` summaries remain only provisional low-latency patches.
- `lib/page-detail-store.ts`: grant-aware renderer cache for versioned Page Detail keyed by Project/Page. One `PageDetailEntry` owns snapshot, subscribers, lifecycle generation, in-flight read, both authority registrations, and LRU state. A visibility fence clears the snapshot before repair, and a failed trailing read cannot restore old content. The read model proves the exclusive Page parent and optional Source membership/schema/value slice; title/body remain exact-head projections edited through the owned Y.Doc surface.
...
- `lib/nodex-y-provider.ts`, `block-document-surface-runtime.ts`, `document-session-registry.ts`, and `owned-block-document*.ts`: transport-neutral Yjs provider, registry-dispatched surface lifecycle, canonical `DocumentSession`, surface leases, and prepared-descriptor validation for one writable Block Document. A renderer shares exactly one Y.Doc/provider for `{storeEpoch, normalized access scope, documentId, generation, schema, sync engine}`; owner display metadata, lifecycle, durable head, tab, and tab group never create a second local authority. Every mounted editor has an independent `EditorSurfaceLease`, BlockNote instance, transaction origin, UndoManager, selection/focus state, and Awareness contribution. The provider owns state-vector synchronization, merged local-update batching, one durable command in flight, idempotent retry, exact-head gap repair, store/generation/integrity reset boundaries, and bounded close. IndexedDB whole-state checkpoints are an observable best-effort recovery lane: they can merge a missing causal diff after Core sync, but their latency or failure never blocks Core ACK or `flushAndFence()`.
...
- `lib/use-workbench-shortcuts.ts`: app-wide Project, command, search, navigation, and panel-command shortcut classification. Contextual commands resolve through one mounted-surface registry whose presentation identity is selected by the Workbench tab/leaf; target updates preserve ownership and only a true unmount disposes the registration. Retired stage/sliding-window keys remain unhandled for the mounted surface.
...
6. Each renderer window owns one LocalCommit delivery registry and one `AuthorityFreshnessIndex` before Query consumers render. Exact-scope projection consumers attach a `CausalProjectionRuntime`; it accepts only a contiguous `base_revision → result_revision`, applies a complete patch synchronously, buffers bounded future effects, detects duplicate-hash divergence, and coalesces canonical repairs for gaps, patchless effects, address resets, or incomplete windows. Authority-bearing canonical reads are admitted only after their Core stamp covers the address and every dynamic root floor; renderer DTO fields never predict those roots. Exact revokes evict matching feature state before any post-state callback, while conservative visibility and transport resets fence the whole address. Kanban's direct Database-row effect updates cards, query rows, per-group windows, totals, and the exact projection coordinate before any repair I/O. Ownership-path and navigation events remain for topology or permission side effects rather than freshness.
...
4. Two Page tabs or tab groups opening the same authorized Document share the canonical Y.Doc/provider but never their editor interaction state. Each tab-keyed `EditorSurfaceLease` retains its own BlockNote editor, surface-tagged Yjs undo history, and Yjs-relative cursor while its React body and EditorView are absent. Switching away removes only that surface's aggregated Awareness contribution and backgrounds a bounded persist; another active surface keeps presence and sync alive. Returning mounts a fresh EditorView over the same Y.Doc. The final lease closes the DocumentSession; store epoch, access scope, generation, or schema changes create a new session, while owner metadata and head advancement do not. Renderer close flushes every ready retained DocumentSession once.
...
6. The relocation Hub makes `Move to Page` a stable-ID, dual-Document transaction. Each consumed source calls `flushAndFence()`, which waits only for pending provider updates to receive a Core durable ACK and returns the exact store/generation/head. IndexedDB checkpoint work stays parallel and the editor does not freeze or expose a structural pending projection. Core validates every fence before committing the structural mutation; response loss resolves through the immutable receipt, exact authorized Document effect, durable stream replay, or state-vector resync.
...
10. Generated items and server requests have a shared protocol-backed raw-state boundary that preserves complete discriminated envelopes and scalar request identity. `codex-conversation-reducer.ts` owns the pure raw `item/started` / `item/completed` transition plus explicit streaming/collaboration effects and the shared admission/timing kernel consumed by both main/no-owner and renderer-owner adapters: same-ID stable upsert, authoritative final replacement, exact-type completion guards, hidden lifecycle identity, and turn-owned command timing are no longer independently defined by those transports. `codex-turn-lifecycle.ts` owns canonical turn start/completion, including nullable placeholder rebind, launch-context synthesis, strict terminal mutation, stale plan retirement, and completed-plan follow-up creation. `codex-turn-metadata.ts` owns diff, safety buffering, hook occurrences, plan rows, review/guardian rows, model reroutes, and notification errors. Both main and renderer-owner ingress commit those canonical turn transitions before applying the same scoped projection; main keeps owner-routed recovery silent and publishes no-owner changes only after the canonical commit. A renderer-owner snapshot carries the lossless canonical document itself; owner item and turn lifecycle reduce that document first and apply a scoped canonical-to-view diff only after the existing queue drain. Missing or mismatched canonical state fails closed into the normal ownership recovery path instead of reconstructing raw turns from transcript rows. `codex-frame-text-delta.ts` and `codex-frame-text-delta-queue.ts` likewise own the four raw prose/reasoning append targets, flush-time turn selection, exact raw-type lookup, and one manager-global 16ms/24-unit/eight-drain-frame scheduler. `codex-server-request-lifecycle.ts` owns request ingress, server withdrawal, local response, and request-caused synthetic-item transitions for canonical replay plus both transport owners. Its queue is an arrival-ordered sequence of complete envelopes: duplicates remain distinct occurrences, numeric and textual request ids compare strictly, ingress raises the request-side `hasUnreadTurn` flag, and removal never clears that flag. A withdrawal uses the first strict match to complete any permission/user-input/MCP synthetic item, then removes every envelope with that same scalar id without emitting a client response. Renderer-owner ingress, withdrawal, and local replies reduce this canonical request document before deriving request cards, attachments, and stream patches; outward request listeners run only after the committed conversation is readable. Hydration/live materialization clones only the image-generation and collaboration protocol exceptions, while live context compaction adds its app-owned completion/source fields. Main and renderer still project raw reducer results into `CodexItemView`, `CodexTranscriptEntry`, `CodexConversationSnapshot`, and request-card view models while C-08 removes the remaining derived state adapters and completes history/transport convergence, and Q-02 alone owns exact direct-request projection, placement, blocking, and UI action surfaces; those flattened snapshots are not the raw-field authority. Thread summaries and snapshots carry a minimal `source` contract: child/helper threads set `source.parentThreadId`, side chats set `source.sideConversation`, and root threads keep `source = null`.
   Renderer-owner prose/reasoning frame batches, command-output batches, and
   parsed terminal commands mutate the handed-off canonical document before the
   scoped projection. Their manager-global queues, UTF-16 segment accounting,
   output truncation policy, and partial terminal-input buffers remain transport
   state. Every material canonical change enters the serialized owner publication
   outbox; the forwarded notification sequence is acknowledged only after main
   accepts that publication. Main may reduce the same raw event into its dormant
   recovery record, but it cannot advance the accepted active-owner replica at the
   same revision.
   Renderer-owner file-patch updates and MCP progress structural repairs follow
   the same rule: they reduce canonical turns first, apply the scoped projection,
   and publish any material change through the owner outbox. Legacy file output
   bytes and reasoning summary-boundary notifications remain explicit no-op ACK
   cases; they do not fabricate a transcript mutation.
   ...
   Visible turn activity then follows one renderer-only presentation pipeline:
   canonical semantic items -> turn buckets -> activity classification -> immutable
   raw units -> parent global/main-slice/fallback state -> per-group state ->
   settled-singleton demotion -> typed completed header plus independently filtered
   body entries. `thread-activity-presentation.ts` is the sole production
   orchestrator; React consumes its units and cannot rebuild topology. Reasoning is
   retained as parent fallback input but hidden from tool membership, body, and
   completed facts. A render-revision selector observes the canonical turn/item
   identities and mutable lifecycle/content evidence, invalidating cached turn
   models without introducing a second projector.
...
12. Main keeps a manager-owned `codex:host-message` plane for shared objects,
sidebar updates, targeted owner notifications, owner/follower control messages,
and accepted thread-stream state. Every Electron window registers as a renderer
client. The renderer-client router targets the active conversation owner, sends
control messages to exact clients, and relays accepted state only to acknowledged
followers while excluding the source owner.
   Owner-routed app-server notifications carry a monotonically increasing
per-conversation sequence. The renderer owner reduces the shared canonical
document and updates owner-visible state first, then a single serialized outbox
publishes snapshot or patch mutations through
`codex:thread-owner:stream-state:publish`. Main acknowledges the forwarded
notification only after that publication is accepted; publication failure repairs
from main's current accepted snapshot or marks the owner state recoverable.
   Accepted replication uses one content-addressed checkpoint containing protocol
version, owner epoch, revision, and deterministic canonical SHA-256. Main applies
each owner publication as compare-and-swap against the exact base checkpoint,
validates source ownership and epoch, requires contiguous revision, applies the
patch, verifies the resulting hash, and only then replaces its accepted replica
and broadcasts the change. Durable read state is excluded from this shared hash
and remains main-owned.
   Every accepted stream relay and role-tagged resume/adoption result carries that
checkpoint as a mandatory contract field. Both an adopting owner and a follower
verify the returned revision/hash and retain the exact shared resume/snapshot
document as the accepted publication or patch baseline before deriving normalized
renderer presentation. The owner's first optimistic publication therefore diffs
from Main's actual accepted replica, including any normalization delta, instead of
assuming that presentation is the CAS base. Render normalization never becomes
patch or hash authority, and no renderer synthesizes an epoch-zero compatibility
checkpoint.
   Follower membership is a control-plane state machine. A new or replaced owner
increments the owner epoch and invalidates every follower snapshot barrier. Main
sends each follower a targeted snapshot, records the sent checkpoint, and excludes
that follower from delta fanout until `codex:thread-follower:snapshot-applied`
matches both the sent and current checkpoint. Followers validate source owner,
epoch, exact base revision/hash, next revision, and post-apply hash. Any gap,
mismatch, patch failure, or transport reset enters
`codex:thread:stream-resync:request` and requires a fresh snapshot; stale owner
publications and stale ACKs cannot cross an epoch.
   State-changing follower actions continue through
`codex:thread-follower:action` to the current owner. Owner actions that need
app-server transport use the owner-scoped allowlisted facade. Start-turn
optimistic/rebind, complete-history loading, edit rollback/resend, fork-from-turn,
queued follow-ups, thread settings/goals, and request replies all publish through
the same owner transaction boundary. No-role renderers resume/adopt first; an
existing owner returns a follower snapshot instead of issuing a second app-server
resume.
   Owner disposal or delivery failure clears only that client's ownership, rejects
matching revision waiters, emits transport reset/owner-unavailable control, and
moves affected renderers to explicit recovery. Active owners are never overwritten
by source-null snapshots. Mounted thread stages report active views; inactive
owners are retained within bounded policy and unsubscribed only after active
runtime/request checks pass.
   `sharedObjectUpdated`, `sidebarSyncUpdated`, `threadTitleUpdated`,
`threadStreamStateChanged`, `threadStreamFollowersChanged`,
`threadStreamTransportReset`, `threadOwnerUnavailable`,
`threadOwnerNotification`, `threadOwnerRequest`, and no-owner `mcpNotification`
remain distinct host-message responsibilities. The renderer has one
local-conversation manager substrate rather than a parallel Codex control reducer.
...
- Local conversation renders live activity through one pure turn-level presentation pipeline over canonical items. Reasoning summaries remain hidden from activity topology, tool facts, completed headers, and group bodies; the parent presentation alone assigns the latest heading to an open group's thinking fallback, a standalone fallback, or no surface. Active patch/command labels, file-change bodies, turn diffs, and Review affordances remain separate concerns. Standalone fallback is presentation state, never a synthetic canonical/bucket/search item, and React consumes projected units without rebuilding topology.
...
- Main-process local-thread replication accepts renderer-owner documents through a content-addressed compare-and-swap contract. Every accepted state has one checkpoint `(protocolVersion, ownerEpoch, revision, canonicalHash)`; a patch must name the exact base checkpoint, advance by one revision, and produce the declared deterministic hash. A first snapshot has a null base. Main replaces its accepted replica only after all checks pass and never emits a source-null visible stream while an owner exists. Durable read state and Nodex-local authorization requests are excluded from the shared hash.
- Follower delivery has an explicit snapshot barrier. A new attachment or owner epoch receives a targeted snapshot and no deltas until it acknowledges that exact current checkpoint. Followers validate source owner, epoch, base checkpoint, contiguous revision, and post-apply hash; mismatch or transport reset requests a fresh resync instead of silently dropping into a stale state. Owner replacement increments the epoch, fencing stale publications and stale ACKs. The renderer-owner publication outbox serializes canonical mutations and acknowledges forwarded app-server notifications only after Main accepts the resulting checkpoint. Main's copy is a recovery/relay replica, not a second transcript writer, and active-owner cache maintenance cannot mutate the accepted document at the same revision.
- Ordinary renderer actions still pass one owner/follower/resume authority gate. Owners reduce and publish assistant, plan, reasoning, lifecycle, request, file-change, output, terminal, and app-owned canonical mutations; followers route mutations to that owner; no-role renderers resume or adopt first. Renderer-private presentation overlays are stripped before publication. Start/steer dispatch can proceed after optimistic owner visibility, while complete-history replacement, rollback, repair, and follower attach use explicit revision/checkpoint waits. Owner-routed requests remain pending main-process broker entries until handled, and owner loss replays unresolved occurrences only after a new epoch is established. Hidden owners remain cleanup candidates only when they have no active view, runtime, in-progress turn, or pending request.
