...
Workspace events carry an explicit Project-catalog invalidation bit in addition
to affected Project, Session, and Thread identities. The event bridge maps only
catalog mutations to the global Project subscription and routes Session/Thread
mutations through their scoped subscriptions, so one aggregate event never
...
fresh restricted `.generated` Profile and reports the reopened v86 Store's
...
occurrence surface. Store Administration owns v86 readiness, backup listing,
...
semantically validates the complete v86 Document/Canvas/projection/managed-asset
...
3. Main starts or reuses Core before opening the Profile. Every production capability enters its owning native deep Module; Electron never opens SQLite or reconstructs the transaction. Migration conformance uses exact published legacy inventories, the frozen final TypeScript v84 schema artifact, and disposable copies. A Page editor sends binary Yjs updates; there is no Page title/body snapshot command or main-process SQLite fallback. Agent reads publish no mutation events.
4. Core captures domain events in the same transaction as their semantic mutation and publishes only after commit. Every committed Page Document head stores its Library/Page and optional Database/Data Source impact in the durable Owned Document event beside the exact-head materialization; replay reconstructs that recorded impact rather than inferring it from current ownership. Main maps this effect—and the affected Page identities carried by Library, Database, and Automation events—to versioned, membership-independent `page-target-changed` invalidations with the Core store epoch and change-log sequence. Lifecycle, location, and target-visible metadata changes therefore share one Page projection seam. Database mutations additionally emit `database-changed` or compatibility `board-changed` events for their own relational effects. Project Workspace events distinguish a Project-catalog mutation from Project identities that merely scope a Session or Thread mutation. Main republishes each event only to matching Library and Project subscriptions after the durable ACK boundary. A retained-event gap publishes `authority-resync`, dirties every canonical Page/Database reader, and reopens the Core stream from the reported event head; an unexpected stream end reconnects from the last successfully published sequence.
...
6. Renderer shared Project stores (`kanban-store`) receive IPC/SSE Database, compatibility Board, Page-target, and authority-resync events. A Page-target event invalidates matching Database View/Page-detail/reference readers, which reread the canonical exact-head projection instead of accepting a title/summary payload from the event; an invalidation arriving during an initial or background read schedules one coalesced trailing reread. TanStack Query consumers use the same explicit initial-fetch guard instead of relying on ordinary invalidation dedupe. Browser Project and Library SSE reconnection emits authority-resync so disconnect-time changes converge without waiting for focus. Compatibility Board events carrying a `DatabasePageSummary` may still patch the local summary cache directly for their own mutations, while structurally ambiguous events coalesce into a summary refetch per Project. Page target queries dispatch by stable Page identity and invalidate only the exact TanStack Query key; deleted targets retain their Project subscription for restoration. Page Stage breadcrumb queries read the root-to-parent ownership chain from one main-process Page hierarchy/authority snapshot rather than session navigation state and observe the target plus returned ancestor identities. A Library-scoped, identity-free ownership-path event also invalidates their Project prefix after location/lifecycle changes that make the changed Page fail post-change authorization; Project-scoped access events cover grant changes without exposing Page identities or titles. Workbench sidebar session rows receive IPC/SSE session-change signals and refresh affected lightweight project-session summaries; full session detail is loaded through explicit selected-session or panel/tab paths.
...
1. Core accepts the exact published v26, v57, v68, v82, and v83 TypeScript inventories as historical import sources, the exact final TypeScript v84 inventory as its direct handoff, and exact Rust-owned v85/v86 stores. For a historical source, Core identifies the complete normalized physical inventory, takes an online database snapshot plus a validated asset-tree backup, and invokes the bundled hash-pinned migrator only inside a staging Profile until it reaches exact v84. Core then reconstructs authoritative Yjs content through Yrs, rebuilds only derived projections, validates the complete v84 handoff, and atomically publishes Rust ownership as v86 under a crash-recovery journal. A Rust-owned v85 store is validated exactly, backed up, and atomically upgraded with Agent execution profiles. Unpublished same-version lineages, near-matches, ambiguous owners, and future stores fail closed; a Rust-owned v86 store is likewise validated exactly and is never silently repaired.
