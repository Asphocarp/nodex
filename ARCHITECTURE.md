...
- `codex/codex-session-store.ts`: reads current session artifacts from the Nodex-owned `$INTERPRETER_HOME` at `${NODEX_HOME}/agent` first, then read-only falls back to an explicit `$CODEX_HOME` or `~/.codex` for legacy Codex transcript recovery. It supports JSONL rollout materialization and rebuilds visible transcript state from replay-safe events instead of raw bootstrap messages; legacy storage is never used as the new runtime's writable home.
...
4. Core captures domain events in the same transaction as their semantic mutation and publishes only after commit. Every committed Page Document head emits membership-independent `page-target-changed` by Project access context and Page identity; lifecycle, location, and target-visible metadata changes use the same invalidation boundary. A Database-row summary additionally emits `board-changed` when that optional projection exists. Project Workspace events distinguish a Project-catalog mutation from Project identities that merely scope a Session or Thread mutation. Main republishes each event only to the matching global or scoped subscription after the durable ACK boundary.
