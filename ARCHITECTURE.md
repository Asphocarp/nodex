...
`WorkbenchSceneSnapshot` has one semantic owner-root primary surface plus
resource descriptors and normalized right/bottom split trees; those trees are
the only panel placement and ordering source. An owner is an exact Project,
Session, or standalone Library resource root. A Project owner uses the current
...
Resource owners use their top-level Page, Database, or Canvas as the protected
primary and reuse the same tab, split, preview, persistence, and Back/Forward
transitions without selecting a Project or creating a Session. Their content
descriptors carry explicit trusted-Library access context; Scene ownership is
presentation, not authorization. Every Scene persists shared composer-overlay
visibility. Project Scenes also
...
The Window Session catalog persists complete Workbench layout v6
...
- `components/workbench/workbench-runtime.tsx`: the selected Workbench integration Module. It composes Window State, the Session Catalog, owner-scoped Project/Session/Resource Scene materialization, the Panel Controller, Sidebar, Chrome, route, command palette, and selected hosts without becoming the authority for Query, Codex, Browser, Terminal, or editor data.
...
- `components/workbench/*`: the shared Project/Session Scene frame, recursive right/bottom panel group tree, shared leaf-level panel tab strip, renderer-local auxiliary surfaces, DB view host, Page Stage/Canvas Stage/terminal tab wrappers, settings surfaces, Process Manager dialog, and focused Workbench Modules.
...
- `components/workbench/sidebar-pages-section.tsx`, `resource-scene-breadcrumb.tsx`, and the shared Workbench Page/Database/Canvas surfaces: the Project-independent Pages navigation projection and Resource Scene presentation. Pages is a flat list of standalone top-level roots, not a second ownership tree or Library Home. Row activation resolves the canonical Library path, selects that root's Scene, and keeps nested Pages/Views in its ordinary tab tree. `components/library/*` retains only creation and explicit resource actions; Library remains the domain authority rather than a user route. The shared sidebar drag context interprets root reordering as Library ownership moves and Page/Database-to-Project drops as confirmed grants.
