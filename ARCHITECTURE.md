...
- `block-transfer-ipc.ts` and `block-transfer-http.ts`: equivalent trusted Adapters for logical same-Library parent Move/Copy. They replace caller audit/scope identity, evaluate Project access to source and target, and return binary Document commits natively over IPC or as bounded base64 over HTTP. Desktop IPC selects the native Library Block-transfer Adapter under Rust authority for ordinary Page/Document subtrees, ordinary-root promotion/wrapping into Library or Data Source Pages, and existing Page moves across Page Documents, Library, and Data Sources. Existing Page moves reuse the Database membership/value/View-position kernel, update only the containing Page shell Documents, and never rewrite the moved Page's owned Document. The loopback HTTP path and recursive Page-copy ownership closure remain on their existing boundary until the corresponding Core client/compiler slices are enabled.
