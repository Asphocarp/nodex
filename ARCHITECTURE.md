...
- `block-transfer-ipc.ts` and `block-transfer-http.ts`: equivalent trusted Adapters for logical same-Library parent Move/Copy. They replace caller audit/scope identity, evaluate Project access to source and target, and return binary Document commits natively over IPC or as bounded base64 over HTTP. Desktop IPC selects the native Library Block-transfer Adapter under Rust authority for ordinary Page/Document subtrees, ordinary-root promotion/wrapping into Library or Data Source Pages, and existing Page moves between Library and Data Source parents. Those existing Page moves reuse the Database membership/value/View-position kernel and do not rewrite the Page's owned Document. The loopback HTTP path and Page ownership moves involving a containing Page Document remain on their existing boundary until the corresponding Core slices are enabled.
