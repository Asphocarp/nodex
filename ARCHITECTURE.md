...
invalidation. The legacy public Document owner commands for Canvas do not
...
the Session while the live Canvas summary refreshes its displayed title. The
Window Session catalog persists complete Workbench layout v3
...
- `components/workbench/*`: recursive right/bottom panel group tree, shared leaf-level panel tab strip, renderer-local auxiliary surfaces, DB view host, Page Stage/Canvas Stage/session-terminal tab wrappers, settings surfaces, Process Manager dialog, and focused Workbench Modules.
