...
v84 SQL artifact is import evidence, not an executable fallback. The detached
...
post-commit Document cache/publication recovery, exact v84 import, online backup,
...
3. Main starts or reuses Core before opening the Profile. Every production capability enters its owning native deep Module; Electron never opens SQLite or reconstructs the transaction. Migration conformance uses the frozen final TypeScript v84 schema artifact and disposable copies. A Page editor sends binary Yjs updates; there is no Page title/body snapshot command or main-process SQLite fallback. Agent reads publish no mutation events.
4. Core captures domain events in the same transaction as their semantic mutation and publishes only after commit. Every committed Page Document head emits membership-independent `page-target-changed` by Project access context and Page identity; lifecycle, location, and target-visible metadata changes use the same invalidation boundary. A Database-row summary additionally emits `board-changed` when that optional projection exists. Main republishes both only after the durable ACK boundary.
...
1. The frozen final TypeScript v84 schema is the only accepted pre-Core import boundary. It contains the complete Profile/Library, Project/Workspace, Database, Document, Agent, schedule, and projection model, and contains no Nodex transcript-search projection. Core validates its exact non-shadow physical inventory before creating a backup or mutating it, then publishes Rust ownership as v86. A Rust-owned v85 store is validated exactly, backed up, and atomically upgraded with Agent execution profiles. Older, ambiguous, drifted, or future stores fail closed; a Rust-owned v86 store is likewise validated exactly and is never silently repaired.
