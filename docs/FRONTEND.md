...
- Workbench app-shell parity: keep the side-panel toggle in the fixed global header, keep right-panel tab creation plus expand/restore in the right-panel toolbar-height tab header, keep bottom-panel tab creation in the bottom tab header, keep the thread-frame toolbar offset at `0px`, and implement right full-width mode by collapsing only the main thread viewport while the bottom panel remains visible.
