...
- Workbench sidebar: project/session sidebar chrome lives in `src/renderer/components/workbench/codex-sidebar.tsx`; use the shared row classes and markers (`app-shell-left-panel`, `data-app-action-sidebar-section*`, `data-app-action-sidebar-project-*`, `data-app-action-sidebar-thread-*`, `data-app-action-sidebar-thread-pinned`, `data-app-action-sidebar-thread-unread`, `h-token-nav-row`, `px-row-x`, `py-row-y`) instead of local project-row styling, and keep project folder selection inside the project row actions menu.
