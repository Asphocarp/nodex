...
- Thread summary panel parity: `WorkbenchShell` owns the pinned-open preference, while `ConnectedThreadStage` renders the summary action in the thread header's trailing action rail. Match Codex Electron labels and modes: `Toggle pinned summary` while the right panel is collapsed, and `Toggle summary` as a dismissible popover trigger while the right panel is open. The fixed global `header-shell-slot` is populated only from registered bottom/side panel header actions, but reserves the regular right-panel width when that panel is open so the thread header rail ends at the thread/right-panel boundary. When the right panel is collapsed, the thread header layout reserve must use the measured global slot width directly; the visible spacing to the global bottom/side rail comes from the thread header's `pe-1.5`, while the no-drag hitbox may add that same one-gap width without pushing layout farther left. Because the thread frame has a 1px top border and the fixed global header does not, the summary action rail also carries a 1px upward visual offset so its 28px button center matches the global toolbar controls. The panel UI lives under `features/local-conversation/view/summary-panel`, renders pinned mode through `LocalConversationStageScreen.floatingContent`, keeps collapsible section chevrons trailing and hover/focus revealed, and positions the 300px pinned overlay below the thread header instead of overlapping it.
