...
- Panel chooser parity: keep empty-panel actions and panel-header plus menus driven from the same `PANEL_NEW_TAB_ACTIONS` registry in `WorkbenchShell`. Right defaults are `Files`, `Side chat`, `Browser`, `Review`, `DB View`, and `Card Stage`; Terminal defaults to the bottom panel. `db_view`, `review`, and `browser_placeholder` are session singletons and must be filtered/focused consistently across clicks, menu actions, and shortcuts.
