...
### Guardian approvals parity depends on config resolution and reviewer collapse, not a renamed local mode
Codex Electron's Guardian approvals preset is not a cosmetic label on top of Nodex's old per-project sandbox toggle. The stable contract is main-owned resolution from `config/read` plus `configRequirements/read`, then one reviewer-aware mapping layer: `auto` and `guardian-approvals` both resolve to `workspace-write + on-request`, and only `approvals_reviewer` differs (`user` vs `guardian_subagent`). In Nodex, regressions came from treating the selector as renderer-local preference state, from persisting `sandbox` as a canonical mode id, and from forgetting the hard `guardian_approval` gate. Keep permission state config-backed, keep `custom` as the escape hatch for raw states that do not round-trip to a fixed preset, and always collapse `guardian_subagent` back to `user` when the feature gate is unavailable.
