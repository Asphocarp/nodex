...
The Electron workbench window relies on macOS `vibrancy` plus a transparent BrowserWindow background. The main workbench sidebar and the rounded gutter beside the `main-surface` pane should therefore stay renderer-transparent so the native material shows through. Do not put `window-fx-sidebar-surface`, `bg-token-side-bar-background`, `bg-(--background-secondary)`, or similar full-height background classes on the main workbench/session sidebar shell or its floating collapsed wrapper. Those classes resolve to opaque theme colors and cover the native vibrant material; keep only row-level hover/active tints and the opaque `main-surface` foreground pane.
