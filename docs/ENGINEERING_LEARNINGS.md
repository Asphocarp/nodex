...
App-window Back/Forward belongs to the workbench history stack, not to browser-sidebar webview history. Browser-sidebar navigation has its own webview `canGoBack` / `canGoForward` and `goBack` / `goForward` path; do not feed that state into the top-left app chrome. In Nodex, renderer buttons, command palette commands, keyboard/mouse shortcuts, and native menu IPC must all call the same window-local workbench history callbacks so disabled state and navigation targets remain coherent.
...
Mounted turns are not raw chronology, but the renderer also must not freely re-sort turn items. The canonical conversation snapshot already arrives in the correct per-turn order, and the renderer should only apply semantic projection on top of that order: inject a `worked-for` divider before the final assistant anchor when qualifying prior work exists, keep pre-final assistant commentary in the agent-work body, and only treat the last eligible assistant item as the assistant anchor. Re-sorting `conversation.turn.items` in the renderer or collapsing all earlier assistant chunks into `postAssistantItems` breaks reopened long-running threads immediately.
