...
Codex Electron also does not rely on one generic expandable transcript wrapper. The reusable piece is the measured-height hook (`VQ()`), while each transcript subtype still owns its own `motion.div` or `AnimatePresence` wrapper, open-state rules, and pointer/overflow contract. In Nodex, keep `MeasuredExpand` reserved for local fallback surfaces that do not need Codex parity. For transcript surfaces with Codex counterparts, reuse the measured-height hook and explicit Motion wrapper instead of routing them back through a shared accordion abstraction.
