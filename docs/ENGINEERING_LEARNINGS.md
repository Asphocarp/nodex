...
That same shell rule continues one level deeper for queued follow-ups and pending steers. Codex does not hand raw queued-message transport objects to JSX and then let the component improvise drag handles, tooltips, or action labels. It shapes those rows first, then renders them through one dedicated lane family (`hve`, `_ve`, `vve`, `yve`). In Nodex, regressions came back whenever queue rows were treated like generic cards or native `draggable` list items: the lane widened, row chrome drifted, `Worker 1`-style local headers appeared, and `Cmd/Ctrl+Enter` queue semantics stopped matching the visible row affordances. Keep queue/pending-steer rows projection-owned and lane-owned.
...
Codex keeps both above-composer hosts in the same footer-width stack as the composer shell: `above-composer-portal` for fixed transcript-owned surfaces, and `above-composer-queue-portal` for queued steers, queued follow-ups, background terminals, and background agents. Both hosts are normal-flow siblings immediately before the shell. Moving either host into the scroll body or rendering the queue/background stack inside `LocalConversationComposerShell` drops those surfaces below the composer and breaks the Codex width/padding contract.
