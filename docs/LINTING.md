...
| `oxc/no-map-spread`                                                                      | Immutable projection updates through `map` plus object spread are the normal renderer/shared pattern; hot paths need measured, local optimization.                             |
...
`typescript/no-unnecessary-type-arguments`, React key/default-prop/context
rules, caught-error preservation, and the remaining performance rules. Their
current output is bounded and points to potentially meaningful improvements.
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `eslint/no-extra-boolean-cast`, `eslint/no-unreachable`, `eslint/no-unsafe-finally`, `eslint/no-unsafe-optional-chaining`, `oxc/const-comparisons`, `typescript/no-misused-spread` | Repository-wide high-confidence correctness failures | Rewrite the control flow or operation so its runtime behavior is explicit. Tests may opt out of the two unsafe rules when probing nullable/failure behavior.              |
