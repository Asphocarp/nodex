...
- [x] (2026-07-16 04:45Z) Completed the transcript observability follow-up: canonical projection now retains every visible dynamic call's result/outcome/duration; all rendered calls expose formatted Details and exact Raw data; and `nodex_app` rows add semantic compact labels plus bounded NFM insert/patch/replacement previews.
- [x] (2026-07-16 05:00Z) Validated the follow-up with strict typecheck/lint, 1,181 Node tests, 1,323 main-process tests, 29 integration tests, 52 focused renderer tests, and the activity-group suites. The full renderer run passed 2,862 tests; its only failure was an unrelated Kanban history dialog's one-second wait under full-suite load, and that file passed 6/6 immediately in isolation.
...
- Observation: the app-server canonical dynamic item already carried the debugging state the transcript needed, but the local projector retained result content only for two host tools.
  Evidence: the generated `dynamicToolCall` item includes arguments, status, content items, success, and duration, while `projectDynamicToolCall` previously copied content/success only for `create_thread` and `handoff_thread` and dropped duration for every tool. Preserving the canonical item removes the need for a second logging channel.
...
- Decision: retain complete dynamic-call state canonically, then derive transcript semantics without changing or supplementing execution data.
  Rationale: one shared inspector can expose exact arguments, canonical receipt, status, duration, and raw item for every namespace, while a pure `nodex_app` presentation projector can add intent labels and NFM change previews. Exact NFM patches provide honest before/after lines; insert and replacement operations show only known addition/replacement content and never invent missing before-state.
  Date/Author: 2026-07-16 / user direction and Codex during the transcript observability follow-up.
...
The transcript follow-up closes the remaining observability gap. Dynamic rows now preserve the app-server's authoritative completion payload instead of selectively dropping it, reuse the same code/raw visual primitives as MCP calls, and keep existing specialized row interactions outside the inspector button to avoid nested controls. Nodex calls remain compact but identify their real target and effect; NFM edits provide a bounded, source-honest diff preview before the full arguments/receipt/raw inspection layers.
...
Update `ARCHITECTURE.md`, `docs/product-specs/nodex-product-spec.md`, `docs/SECURITY.md`, `docs/RELIABILITY.md`, `docs/references/nested-markdown-spec.md`, Storybook, and this plan. Add an `Unreleased` changelog entry because Agent-readable/writeable Project data is a release-note-worthy capability. Document intent-first tool wording, opaque revision semantics, Card-first search, NFM create/insert/patch/replace rules, ownership separation, scope binding, grant lifecycle, idempotency, headless policy, and old-task rollout limitation.
...
Successor note, 2026-07-16: The unreleased v1 concurrency contract recorded here was replaced by ADR 0015 and `docs/plans/nodex-agent-semantic-preconditions.md`. `nodex_app@2` keeps the intent-oriented tool set and canonical mutation kernels while replacing public storage-revision topology with operation-scoped ETags, validator-free additive NFM operations, on-demand `prepareFor`, sparse results, and post-consent semantic re-resolution. This document remains the historical implementation record for v1 rather than a current API reference.
