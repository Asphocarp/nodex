...
- Observation: current Codex dynamic tools do not provide structured Code Mode results.
  Evidence: Nodex returns one JSON `inputText` item, the generated `DynamicToolFunctionSpec` has no output schema, and `DynamicToolCallResponse` has only `contentItems` and `success`. Pinned Codex 0.144.4 and current upstream convert the dynamic result to a JavaScript string.
