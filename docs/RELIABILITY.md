...
- Sidebar sync is continuous reconciliation, not a one-shot background refresh. Main coalesces concurrent sync calls across windows, uses a 5-second stale gate for `policy:"stale"`, backs off app-server failures from 2s up to 60s, and returns the last SQLite snapshot without throwing when sync fails. `policy:"force"` bypasses stale/backoff gates for mount, project/source changes, and forced unknown-notification repair.
