...
The initiating renderer admits its authorized apply-response delivery before the feature Promise resolves. Other renderers converge through the scoped live broker and durable replay. Main's Projection delivery Module owns the multiplexed physical lease, replacement attempts, callback ingress, backoff, and release as children of the Main Scope; audience membership supplies desired scopes but does not own a connection or retry loop. Apply delivery and later stream delivery are complementary copies of the same committed fact, not competing authorities.
