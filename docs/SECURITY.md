...
- `nodex_app` reads and writes derive Project scope from the verified launched task; model arguments cannot select another Project, store epoch, or catalog revision. Digest-only ETags bind guard kind, Project, store epoch, resource identity, and current semantic state without disclosing that state. Self-contained cursors use a separate signed format. Tampered, cross-resource, cross-Project, post-restore, and ABA membership reuse fail closed.
- Every `nodex_app@3` write performs mutation-free canonical preflight before consent, then re-resolves current authority after consent. Execution proceeds only when the fresh effect class, target resources, deletions, and ownership transformations equal the approved authorization footprint. Authorization previews use bounded `markdownPreview` content for the active contract; historical `nfmPreview` is read only when rendering old records. Authorization therefore covers resolved effects—including identity deletion and protected owners—while incidental Document heads may advance and canonical optimistic concurrency still protects the final transaction.
