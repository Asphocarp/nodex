...
Physical Module events are private reconstruction evidence, never an authorization unit. Store v109 seals closed typed DeliveryAtoms whose descriptors contain the exact canonical `ResourceKey` requirements extracted from their payload. Core sends an atom only when the packet's post-state authorization scope can read every requirement. A mixed business result that has independently visible portions is compiled into independently redacted atoms before Manifest sealing; Main and renderer never trim fields or infer claims. The packet's authorization scope, coverage, resource-atomic descriptors, Document refs, Projection effects, and revocations are covered by packet integrity. Apply, scoped-live, and durable ingress use the same packet-v3 structural validator and fail closed on legacy shape, malformed capability, coverage mismatch, or Store/Library/event mismatch. Revocation remains an independent scoped lane: it evicts stale client authority but does not replace Core read authorization.
