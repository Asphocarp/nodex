...
Direct new-thread creation prepares the same input and client identity before transport and adopts the actual app-server thread as the route identity. Main first hydrates the response into the canonical dormant snapshot, then owner adoption atomically turns that snapshot into the first accepted renderer replica and checkpoint. The first adoption must not require an accepted replica as its own precondition; absence of a canonical snapshot fails closed. Main does not publish the dormant document as a visible source-null stream or add a separate transcript-only user row after the response.
