...
| Resume/start/history lifecycle         | complete | Resume and fresh launch expose one renderer attachment lifecycle, install role/checkpoint before visible notification, terminate failures explicitly, release the resume buffer before the owner snapshot, and keep complete-history revision waits owner-published. |
