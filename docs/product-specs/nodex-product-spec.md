...
- Sidebar project rows do not show the source path inline. Each project row actions menu exposes rename, choose icon, edit sources, add source folder, project pin/unpin, `Open in Finder` for the primary source, and `Remove project`. Removal is available independently of the Library workspace release gate.
...
- `Project sidebar options` always exposes `Removed projects…`. Its lazy manager lists archived Projects, their source roots, and per-row restore actions. Restore preserves the Project ID, Sessions, Threads, tabs, and source configuration, increments the binding revision, recomputes current access, and appends the Project to active Project order without reordering survivors. If the active Project is removed, selection moves to the adjacent surviving Project; after the final Project is removed the Workbench enters an explicit projectless state rather than inventing a fallback Project. Permanent content deletion remains a separate Library resource operation.
...
- Projectless sessions have `projectId: null`, can open the thread stage, rename, archive, pin, fork in the same directory, and own Browser tabs like normal chats. Selecting one enters the explicit no-Project Workbench state, and shell Back/Forward restores that same null Project coordinate. An attached projectless thread can start an ephemeral Side chat; when its cwd is present it can also own Terminal tabs whose PTY starts exactly there. If cwd is absent, Side chat repairs and persists the projectless workspace before forking, while Terminal stays unavailable. A blank projectless session exposes Browser only. Projectless sessions can also own an exact-file Files tab without acquiring Project ownership or a workspace tree root, but cannot own a generic Files tree, DB View, Page Stage, or Review tabs. Persistent same-directory forks and temporary side-chat forks retain the source output-directory and workspace-browser-root hints; the app-server fork payload cannot silently reclassify those children as ordinary project workspaces. Sessions containing only portable Browser/Terminal tabs can move between Project and projectless ownership without changing their session, conversation, panel-row, Browser-tab, or PTY identities.
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/api/projects` | List active projects by default; `?includeArchived=true` includes removed Projects |
