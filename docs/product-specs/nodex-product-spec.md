...
- The global command palette has explicit modes. `Cmd/Ctrl+K` and `Cmd/Ctrl+Shift+P` open root mode, which searches command/action rows only and includes supported shell actions plus disabled Codex-parity mock rows. `Cmd/Ctrl+G` opens chat search, using session-backed, non-archived chat title/preview/project/cwd metadata plus bounded Codex app-server content snippets when available. `Cmd/Ctrl+P` opens card search, using fuzzy full-text ranking across title, description, tags, assignee, agent status, column, project name, and card id. Card mode owns the trailing `Filter` popover and compact active-filter row beneath the input, using the same status/priority/tag/project-style pill language as the DB view toolbar while persisting those filter selections across reopen/reload. A leading `>` no longer switches modes. File search remains a disabled mock row until Nodex has a real file-search backend.
