...
The renderer transport requires the Electron preload bridge and has no browser or localhost fallback. Private Host/Core transport is version 7, committed events are version 7, authorized delivery packets are version 3, and semantic Module versions are selected independently from generated requirements. Ordinary Core JSON requests/responses are capped at 2 MiB/16 MiB, while user-growing collections use at most 200 items and 1 MiB per signed-keyset window; 16 MiB is a fault boundary, not a product capacity target.
