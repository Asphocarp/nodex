...
- Terminal opens as a session-attached bottom-panel tab with a session-tab-scoped terminal id and starts in the attached thread cwd when present, otherwise the owning Project's primary source, otherwise the PTY process default. Pages can request a terminal in a Project execution context, but terminal tabs carry neither Page ownership nor Page IDs. When the terminal backend exits, including a user typing `exit`, the owning terminal tab closes automatically.
...
- Schema v78 stores Profile→Library, Page ownership, Database Container/Data Source/View identity, Project binding/lifecycle/grants, engine-neutral Documents, immutable mutation/history evidence, and rebuildable Page/search/schedule/reference projections. Physical Space/Document/Database coordinates are private adapters for canonical Library/Page/Data Source parents.
...
- Awareness distinguishes mounted windows/sessions and is advisory rather than a lock. Hiding a retained Page Stage clears its presence and closes that surface client; returning creates a new client session and state-vector-syncs any intervening content before editing resumes
...
`get_context`, `search`, and `fetch` are eager; both query tools and every write are deferred. Reads are evaluated against the Project's bound Database plus recursive Page/Database grants inside its Library. A read-write Page grant allows content edits; structural writes remain bound to the Project Database except at the exact root of an explicitly granted Page. Moving or creating a top-level Library Page establishes a recursive read-write grant for the executing Project, so authority does not disappear when the Page leaves its bound Data Source. Ordinary reads return no concurrency validators. A bounded typed `prepareFor` list asks only for the short title/body/Block ETags required by the next semantic write. Writes perform mutation-free semantic preflight before consent, re-resolve current authority afterward, and execute only when the fresh effect/resource/deletion/transformation footprint still matches what was approved. Resource grants and per-call Agent approval remain independent.
