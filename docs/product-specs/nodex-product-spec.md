...
- Drag-handle `Move to` sends stable root Block IDs plus logical `library | page | data_source` parents. Core resolves current physical storage coordinates, validates the target View/Data Source, and commits content, parent, Source membership/value state, View positions, projections, history, document commit refs, and receipt atomically. The receipt's `changeLogSeq` is the causal floor for Board/Page rereads, and open Document providers receive the committed Yjs updates immediately. No renderer removes the source optimistically, renders a structural pending projection, or reads a Page NFM snapshot. Losslessly promotable roots preserve their ID as Page identity; Option/Alt Copy allocates fresh identities.
