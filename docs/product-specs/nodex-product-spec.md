...
- Settings is a full-window Workbench route with one canonical section catalog, local search, feature-owned pages, and explicit in-app deep links. Its route, navigation, grouping, search, composition, and ownership contract lives in [Settings Route Behavior](settings-route-behavior.md).
...
- Scheduled is a Workbench route over Core-owned definitions and runs, with URL-owned selection, task/template discovery, a guarded detail editor, and committed run synchronization. Its complete route and mutation contract lives in [Scheduled Route Behavior](scheduled-route-behavior.md).
...
- After the user stops an ordinary Codex turn, an idle empty composer replaces its circular Send/Stop action with `Resume`. Resume continues the interrupted task as a userless turn with the thread's current settings and never adds a synthetic prompt or user bubble. Typing, adding an attachment, or a saved thread goal returns control to the ordinary Send or goal flow.
...
- A bounded renderer-lifetime Page Detail cache survives inactive durable-tab unmounts. Returning to an unchanged Page presents its cached detail synchronously instead of replaying the hydration skeleton. Database View effects refresh collection readers and the exact changed Page advances through its Page effect; neither complete row patches nor patchless View repairs evict cached sibling Page Details merely because they share a Database or Data Source. Property and option mutations advance a Page Detail Data Source scope, View and Database descriptor mutations advance a Page Detail Database scope, and exact revocations, checkpoint gaps, and resets still fence every dependent cache before reuse.
...
- Calendar presentations are temporarily behind an application-wide, default-off release gate while their interaction design is rebuilt. When the gate is off, the toolbar and new-View authoring omit Calendar, a restored legacy Calendar tab presents the same Database as Board, and an existing durable Calendar View presents its exact query as List without changing its View identity, kind, config, revision, or saved Workbench preference. Existing Calendar Views remain manageable and can be converted to Board/List or deleted. This gate controls only renderer presentation: Page schedule, recurrence, reminders, Page Detail occurrence actions, canonical `calendar` View data, and background reminder delivery remain available.

#### 11. Calendar View
- The behavior below is the gate-enabled Calendar contract and remains the redesign acceptance target. Calendar can be enabled explicitly in development with `VITE_NODEX_CALENDAR_PRESENTATION=true`; production release state is a checked-in decision, not a user setting or remote telemetry flag.
...
- The floating Thread Summary panel is a derived view of one attached root Chat. Its section order, visibility, artifacts, Git actions, auxiliary conversations, process rows, Browser state, sources, and PiP behavior live in [Thread Summary Panel Behavior](thread-summary-panel-behavior.md).
