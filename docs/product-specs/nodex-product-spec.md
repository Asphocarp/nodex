...
- Reference disclosure preference persists in the local browser profile by stable shell Block identity, while visibility and provider activation remain window-local and a small renderer-wide activation budget bounds live nested providers. Separate `cardRef` occurrences targeting the same Card retain independent preferences. Self references do not recursively mount, and cross-Project references use the target Project's workspace context.
