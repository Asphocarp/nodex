...
- Schema v62 stores Block identity with one Space/Document/Database parent, engine-neutral Owned Documents, stable dormant Database membership/properties/Views, immutable mutation/history evidence, rebuildable projections, and a validated portable-rich Card title projection. It also owns durable unread/delegated-thread ordering and metadata, conversation-scoped Browser tab identities that permit Browser-only projectless ownership, and unique project-session ownership for every durable Codex thread. Block-tree owners use Yjs authority; Canvas uses normalized scene authority. The content-bearing Card/history/description snapshot tables are absent.
...
Schema v62 is Block-first. `blocks` gives every active Card one Space, Document, or Database parent; a Database parent has one matching active membership, and each Card/Database pair retains at most one stable historical membership for dormant-value restoration. `retired_block_identities` permanently reserves collected IDs; `documents` plus `block_documents` own independently synchronized content and select its sync engine. Yjs tables own `block_tree` causal state; Canvas scene tables and immutable receipts own normalized `scene_graph` state. Database records model membership/properties/Views without copying Cards, mutation/history evidence is immutable, and read projections remain rebuildable at exact authority coordinates. `project_session_threads` is one-to-one in both directions; the v61-to-v62 migration keeps the scope-consistent active owner with the strongest user-content signal, detaches and archives redundant historical shells without deleting their tabs, and then enforces unique thread ownership. A shipped v26 or v57 store is converted off-line in staging, journal-installed only after the complete v58 candidate passes validation, and then advances through ordered, resumable v59-v62 migrations before becoming observable. The v60-to-v61 step rematerializes readable Card NFM identity/mention syntax at the same Yjs generation and head; a retained deleted Card owner receives the same catch-up inside a later restore transaction before exposure.
