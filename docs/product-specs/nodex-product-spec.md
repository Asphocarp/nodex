...
- A selected Session Scene can show, collapse, resize, or full-width expand its right panel. Project and Resource Scenes instead use the same panel tree as a structurally open, full-width surface stack. A Project's current default Database View and a Resource Scene's top-level Page, Database, or Canvas are their respective first non-closable root tabs and cannot move, split away, or enter the bottom panel. The Project root is projected as the Project marker and `Project Home`; a Resource root keeps its live resource title and type icon. Opening a Page from a Database in a one-group full-width stack creates an adjacent right tab group; later Pages from that Database reuse the nearest right group while the sidebar continues to highlight the owning root. Right, bottom, and split-panel resize drags remain continuous even when Browser webview content is visible under the pointer. The fixed global header keeps eligible panel controls clickable over every surface. When the sidebar is collapsed and a surface stack is full-width, its tab header starts after the measured left titlebar rail so the left titlebar buttons and tabs do not overlap. Newly materialized Session Scenes default to collapsed right panels; newly materialized Project and Resource Scenes normalize to open/full-width, with Project onboarding optionally activating the Welcome Page while retaining Database as the protected first tab.
...
- App-window Back/Forward navigation is available from the top-left titlebar controls, `Cmd/Ctrl+[` and `Cmd/Ctrl+]`, desktop mouse Back/Forward buttons, the command palette, and the macOS application menu. It navigates backward/forward through Window Session-owned Workbench context: the exact Project/Session/Resource/empty location plus every owner Scene's primary, active panel surfaces, collapse state, and right-panel full-width state. First materialization and live title synchronization do not add phantom history entries. Agent Dock binding and composer-overlay visibility updates, settings, command palette, task search, and Browser webview history are not part of this stack.
...
- Each restored window resumes its exact Project/Session/empty location, owner Scene snapshots, active panel surfaces, panel state, composer-overlay visibility, Project Agent Dock binding, open resource context, selected Thread context, Workbench layout, and saved window bounds.
...
- Project-local Session identity, ordering, pin, archive, unread, no-thread fallback label, and optional Thread link are shared domain data in SQLite. A Main-persisted Window Session owns canonical layout v6 with Scene v4: exact location plus owner-keyed `WorkbenchSceneSnapshot` values whose semantic primary, shared composer-overlay visibility, and right/bottom panel trees hold local surface descriptors, collapse, size, active leaf/surface, and ordering. Project Scenes additionally persist Agent Dock binding and unbound draft identity; Resource Scenes persist only their standalone root identity and content-surface layout. Legacy layout v5 Library locations migrate back to their saved `returnTo`, while Project and Session Scene identities and panel state migrate deterministically. Renderer-only state owns previews, transient focus history, and temporary side-chat tabs. Saving uses a monotonic `layoutRevision` with stale-write rejection and atomic file replacement. There is no Library route state, database-starter Session, or parallel per-Session layout map.
...
- Settings -> General -> `Desktop notifications` exposes three independent controls: `Turn complete` (`Never`, `Only when unfocused`, `Always`), `Approval requests` (boolean), and `Questions` (boolean). Defaults are `Only when unfocused`, approvals enabled, and questions enabled. When the OS exposes a determinate notification permission, the same surface reports it and opens the platform notification settings without changing those three preferences.
...
- Pages uses the same section, row, leading-slot, action-rail, focus, active, and pager primitives as Projects. Page, Database, and Canvas use `FileText`, `Table2`, and `Shapes`; the app does not use a cylinder icon for Database. Up to five eligible roots are initially visible. `Show more` appears only when a sixth loaded item or a real source continuation exists, expands in place, and shares Projects' focus restoration and `Show less` behavior. The section has no `Open Library` action and there is no Library Home route. The plus action creates a root Page or Database and immediately opens its Resource Scene.
- Selecting a Pages row resolves the target's canonical Library ownership path and selects one Resource Scene owned by its top-level root. It never selects a Project, creates a Session, or infers navigation from a resource grant. The root is the protected first tab; nested Pages, Database row Pages, selected Views, and referenced Canvases open through the ordinary preview/durable tab and split rules in that same Scene. Selecting another top-level root switches to that root's retained Scene. The sidebar continues to highlight the Scene root while a child tab is active. Explicit `Open in Project` remains a separate grant-and-transition action.
- Resource Scene Page, Database, Canvas, property, and Document operations use trusted local-human Library authority. Public renderer results identify `accessContext: library` and do not expose a private compatibility Project. New Untitled Pages focus their collaborative title. A Database root opens its durable default View unless an exact View was selected, and uses the same Database View controller/presenter, search, group pagination, mutations, Page opening, and toolbar composition as ordinary DB View tabs; there is no Library-specific board. Canvas uses the shared Canvas Stage without selecting its compatibility storage Project.
- The fixed App Header publishes `Pages / <root>` and, when a child tab is active, `Pages / <root> / <child>`. `Pages` is context text rather than a link to a nonexistent Home. Root and child titles update from live resource presentation. App-window Back/Forward and Settings/Automations return transitions restore the Resource location together with its exact active tab and Scene layout.
