...
- Card Stage and Toggle List editors interoperate with Kanban through ordered drag sessions in either direction. Same-window and Electron cross-window drops default to move; Alt/Option at release copies. Editor-to-board moves persist projected source descriptions before removing live blocks, while board-to-editor drops insert detached `cardToggle` snapshots optimistically and roll back on transaction failure. Self-drop is blocked for both operations.
