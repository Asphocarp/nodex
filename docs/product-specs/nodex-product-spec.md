...
Clearing a non-empty Relation requires explicit confirmation and uses a revision-fenced empty replacement, including targets hidden as `Restricted`. Incremental removal and non-empty replacement accept only Page IDs that are currently readable, preventing guessed targets from revealing membership. Saved Relation filter operands are rechecked whenever the View is read after authorization changes. Page Detail built-in Source Properties use the same Database write contract as Database Views. An action that also changes intrinsic schedule/run metadata commits both owning-module mutations atomically.
...
Fresh Profiles use Store v104. Exact v100 Stores are backed up and atomically
migrated by adding the persisted projectless permission-mode scope after
normalized Relation authority, then applying the v102 LocalCommit ledger, the
v103 composite-identity foreign-key cutover, and the v104 canonical evidence
hash cutover. Drifted or partially migrated inventories fail closed.
...
- While a turn is streaming, visible assistant commentary does not by itself end the live activity slice. The latest readable reasoning summary remains independently visible as one gray shimmer line until a terminal, blocking, safety-buffering, non-latest, or unfollowed state closes the slice. An active file-change item displays its activity label (including `Editing files` before any paths or diff body materialize); when no concrete activity group exists, the same summary falls back to a standalone `Thinking` row. This live activity is independent from `Edited files, read files`, turn diff, Review capability, and Projectless scope.
...
- Date and datetime Properties use one controlled six-week calendar with typed input, Today, and Clear. A date persists only `YYYY-MM-DD`; datetime additionally exposes a real local-time input and persists canonical ISO time. Due date does not display range, reminder, or time affordances that its schema cannot store. Relation Properties use a target-Source-aware search popover with separate Selected and candidate groups, explicit add/remove controls, candidate and selected-target pagination, stale-result fencing, and an inline clear-all confirmation. Opening the picker with an empty search lists the target Data Source's accessible active Pages in stable title order and continues through the same bounded cursor window; typing narrows that list through one cursor-stable match key that folds ASCII case and preserves every other Unicode code point exactly. Restricted Relation targets expose only a count, never target identity or title.
...
- Drag-handle `Move to` sends stable root Block IDs plus logical `library | page | data_source` parents. Core resolves current physical storage coordinates, validates the target View/Data Source, and commits content, parent, Source membership/value state, View positions, projections, history, document commit refs, and receipt atomically. The receipt's `commitSeq` is the causal floor for Board/Page rereads, and open Document providers receive the committed Yjs updates immediately. No renderer removes the source optimistically, renders a structural pending projection, or reads a Page NFM snapshot. The apply response publishes its LocalCommit to Main before durable tailing or projection reads finish; a later tailer copy is deduplicated by `(store_epoch, commit_seq)`. Losslessly promotable roots preserve their ID as Page identity; Option/Alt Copy allocates fresh identities.
