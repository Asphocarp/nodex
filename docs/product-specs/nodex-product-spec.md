...
- Sidebar footer keeps a compact Settings button and no workspace switching controls.
...
- In-app account UX supports account read, ChatGPT/API-key login, login cancel, logout, and an authenticated quota row in the top-right summary surface for attached-thread pages. With the right panel collapsed, the row appears in the pinned summary panel below the thread header; with the right panel open, the thread-header `Toggle summary` button opens the same summary surface as a dismissible popover. That row shows concise remaining primary/secondary percentages (for example `82% · 61%`), refreshes account data when the panel/row is revealed, keeps the detailed remaining windows in the tooltip when available, and refreshes quota data in the background every 60 seconds while the Codex connection is live and authenticated. Signed-out auth remains available from the thread header.
