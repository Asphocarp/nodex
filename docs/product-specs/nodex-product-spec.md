...
- NFM supports inline `<mention-thread uuid="..." />` mentions for Codex thread references. They render as minimal Notion-style inline references, resolve local thread metadata when available, open the referenced thread when clicked, serialize to `[Thread: <uuid>]` in copy and thread-section prompts, and never inject the mentioned thread transcript into `promptInput`. The NFM `@` picker uses the same command-palette chat/card search model for thread mentions and card references, including sidebar-wide chat metadata/content search and card metadata/description search, while keeping the stored `threadMention` and `cardRef` shapes unchanged. Detailed behavior lives in [NFM Editor Thread Mention Behavior](./nfm-editor-thread-mention-behavior.md).
