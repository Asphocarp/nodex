...
- Long Codex thread transcripts load the recent history first and request older turns on demand as the user reaches the top of loaded content. Older-page loading preserves the visible transcript position, and rail/find navigation can reveal targets in virtualized or newly loaded history without snapping to the bottom.
