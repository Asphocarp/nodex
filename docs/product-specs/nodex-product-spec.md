...
- Settings can choose which optional card-stage rows start behind the Card Stage `more properties` toggle (`Tags`, `Assignee`, `Threads`, and `Schedule`)
...
- Inline Card and Database View references are reference-only Blocks. Idle collapsed Card rows render summary projections; disclosure or explicit title engagement lazily mounts that Card's independent collaborative Document, never a copied child subtree in the host editor.
- Reference disclosure preference persists in the local browser profile by stable shell Block identity, while title engagement, visibility, navigation handles, and provider activation remain mount-local. A small renderer-wide activation budget bounds live nested providers and gives focused editing priority over passive visibility. Separate `cardRef` occurrences targeting the same Card retain independent preferences. Self references do not recursively mount, and cross-Project references use the target Project's workspace context.
...
- Toggle List row and in-editor toggle disclosure preferences persist in the local browser profile and never enter a Card body or Y.Doc. Active windows keep their own live presentation; reopening or remounting hydrates the latest local preference without producing collaborative updates or undo history.
...
- Schema v63 stores Block identity with one Space/Document/Database parent, engine-neutral Owned Documents, stable dormant Database membership/properties/Views, immutable mutation/history evidence, rebuildable projections, and a validated portable-rich Card title projection. It also owns durable unread/delegated-thread ordering and metadata, conversation-scoped Browser tab identities that permit Browser-only projectless ownership, and unique project-session ownership for every durable Codex thread. Card Agent blocked/status properties are absent because live execution state belongs to Thread/session runtime. Block-tree owners use Yjs authority; Canvas uses normalized scene authority. The content-bearing Card/history/description snapshot tables are absent.
...
- NFM supports inline `<mention-thread uuid="..." />` mentions for Codex thread references. They render as minimal Notion-style inline references, resolve local thread metadata when available, open the referenced thread when clicked, serialize to `[Thread: <uuid>]` in copy and thread-section prompts, and never inject the mentioned thread transcript into `promptInput`. The NFM `@` picker uses the same command-palette chat/card search model for thread mentions and block-level Card mentions, including sidebar-wide chat metadata/content search and Card metadata/description search, while keeping the stored `threadMention` and internal `cardRef` shapes unchanged. Picker results prioritize the editor's current-project chats/cards in a `Current project` group, omit redundant right-side mention syntax hints, and keep hover tooltips to compact context plus optional search snippets. Detailed behavior lives in [NFM Editor Thread Mention Behavior](./nfm-editor-thread-mention-behavior.md).
...
- For child `card` and `cardRef` outliner rows, unmodified `ArrowUp` / `ArrowDown` traverse the visible host Block order, authoritative Card title, and disclosed body Blocks as one sequence. Entry waits for visual textblock boundaries, skips hidden collapsed-toggle descendants, and does not change Card disclosure.
- Typing `## ` inside a toggle header converts it to a toggle heading (preserves toggle state)
- `Cmd/Ctrl+Enter` modifies the current actionable NFM block before any Card Stage send fallback: checklist blocks toggle checked state, toggle list items and toggle headings expand/collapse, image blocks open preview, child `card` and `cardRef` occurrences toggle their local outliner disclosure, and bound `threadSection` markers open their linked thread. Card disclosure works from the selected host shell and the live title header; events originating in a disclosed nested body remain scoped to that body editor. An unavailable Card occurrence consumes the Card modify action without changing its saved disclosure preference or falling through to section send. If no modify action is available, Card Stage keeps the thread-section send behavior without moving focus to the Threads stage; unsectioned content still creates a section marker before sending.
...
- `@Card` inserts a fully resolved Card mention whose internal childless `cardRef` stores only `targetBlockId`. `/card` stays hidden until its picker can choose a target before the Y.Doc transaction; no writable surface may create an unresolved Card mention. Legacy `Toggle List Inline View` insertion is unavailable; migrated inline queries use `databaseViewRef` and a durable View definition.
...
- Card targets resolve from Block/Document content authority even when the Card belongs to no Database. Idle collapsed rows mount no target provider. Clicking the projected title or Arrow-entering from an adjacent visible host Block activates one authoritative target surface and its collaborative Y.Text title while disclosure remains collapsed; the body editor is not constructed. Expansion reuses that surface, keeps one observed row frame and one title row, and renders only the target body at standard Block child indentation. Arrow traversal crosses host Blocks, title, and disclosed body boundaries in visible order. There is no nested panel, second title, vertical divider, or foreign content in the host Block tree.
- Canonical child Card shells persist no title-like or copied identity prop; NFM derives their owning Block identity as `<card uuid="..." />`. Internal Card mentions persist only `targetBlockId`, while NFM exposes the target through `<mention-card url="nodex://cards/..." />`. Historical `<card />`, `<card-ref ... />`, and `display-hint` values are decode-only. Existing Card UUIDs may pin same-Document shells during exact-head NFM replacement, but create/copy/move remain typed ownership operations. Collapsed rows read the exact-head membership-independent title summary; an empty current title displays `Untitled`, never an insertion-time snapshot.
...
- Reference recursion is guarded by an inherited Card ancestry path (including A → B → A), while a per-mounted-surface provider budget caps independent editors and gives an explicitly engaged title priority over visibility-only surfaces; foreign bodies never enter the host tree.
...
- Projectless sessions have `projectId: null`, can open the thread stage, rename, archive, pin, fork in the same directory, and own Browser tabs like normal chats, but they do not switch the active project and cannot own DB View, Card Stage, terminal, Review, or Files tabs. Persistent same-directory forks and temporary side-chat forks retain the source output-directory and workspace-browser-root hints; the app-server fork payload cannot silently reclassify those children as ordinary project workspaces. Browser-only sessions can move between project and projectless ownership without changing their session conversation, panel-row, or logical browser-tab identities.
...
- Renderable web searches remain individual leaves inside mixed activity groups. Each row shows its normalized action/query detail, uses the resolved domain favicon when available, and does not invent a fallback icon. Searches with a blank top-level query are hidden before grouping, even if secondary action metadata contains displayable text.
...
Schema v63 is Block-first. `blocks` gives every active Card one Space, Document, or Database parent; a Database parent has one matching active membership, and each Card/Database pair retains at most one stable historical membership for dormant-value restoration. `retired_block_identities` permanently reserves collected IDs; `documents` plus `block_documents` own independently synchronized content and select its sync engine. Yjs tables own `block_tree` causal state; Canvas scene tables and immutable receipts own normalized `scene_graph` state. Database records model membership/properties/Views without copying Cards, mutation/history evidence is immutable, and read projections remain rebuildable at exact authority coordinates. `project_session_threads` is one-to-one in both directions; the v61-to-v62 migration keeps the scope-consistent active owner with the strongest user-content signal, detaches and archives redundant historical shells without deleting their tabs, and then enforces unique thread ownership. The v62-to-v63 migration removes retired Card Agent blocked/status authority rows and rebuilds Card projections without changing Card revisions or immutable mutation/history evidence. A shipped v26 or v57 store is converted off-line in staging, journal-installed only after the complete v58 candidate passes validation, and then advances through ordered, resumable v59-v63 migrations before becoming observable. The v60-to-v61 step rematerializes readable Card NFM identity/mention syntax at the same Yjs generation and head; a retained deleted Card owner receives the same catch-up inside a later restore transaction before exposure.
...
CREATE TABLE project_session_tabs (
  ...
  project_id TEXT REFERENCES projects(id) ON DELETE CASCADE, -- nullable only for browser
  browser_tab_id TEXT,              -- required only for browser; logical id scoped by session_id
...
The transport layer (`src/renderer/lib/api.ts`) auto-detects the runtime and uses the appropriate path. In Electron, renderer HTTP routes resolve from a preload-injected server URL; in browser mode they resolve from same-origin (with localhost Vite dev fallback to default API port `51283`). Board, Card-target, Database, and session events share one project-scoped stream; project-list/order/pin changes use the global `/api/projects/events` stream. `card-target-changed` carries only Project/target identity plus change kind and optional committed Document coordinates, so mounted consumers invalidate the exact target query and reread validated authority rather than trusting event content.
...
```bash
# Inspect active Card Block identities and locations
nodex query "SELECT id, project_id, location_kind FROM blocks WHERE type = ? AND lifecycle = ?" card active
```

Use `nodex ls` with its status, priority, assignee, pagination, and output options for product-level Card queries. `nodex query` exposes storage-level records for diagnostics and advanced inspection; it does not provide a legacy wide Card table. Only SELECT queries are allowed (enforced via SQLite's `Statement.readonly`), and parameters are positional (`?` placeholders).
