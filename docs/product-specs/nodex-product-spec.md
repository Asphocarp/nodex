...
- Cross-surface drag treats the Kanban Database View as a projection rather than a content container. Dragging one or more Kanban Cards into a visible NFM editor inserts canonical Card references at the pointer without removing Database membership or embedding foreign bodies. Dragging that canonical reference back onto another column moves the referenced Database row while leaving the reference intact. Dragging ordinary NFM Blocks into a Kanban column copies each top-level Block into a new Card through NFM genesis import and leaves the source Y.Doc unchanged; the cursor and insertion cue explicitly communicate copy/link semantics. Existing Card Documents are never reconstructed from a drag payload, and destructive promotion is not implied by this gesture.
