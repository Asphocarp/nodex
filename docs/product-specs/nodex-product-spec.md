...
- Same-window cross-surface drag treats the Kanban Database View as a projection rather than a content container. Dragging one or more Kanban Cards into a visible NFM editor inserts canonical Card references at the pointer without removing Database membership or embedding foreign bodies. Dragging that canonical reference back onto another column moves the referenced Database row while leaving the reference intact. Dragging ordinary NFM Blocks into a Kanban column copies each top-level Block into a new Card through NFM genesis import and leaves the source Y.Doc unchanged; the cursor and insertion cue explicitly communicate copy/link semantics. Existing Card Documents are never reconstructed from a drag payload, destructive promotion is not implied by this gesture, and cross-window DnD is intentionally unsupported.
- Kanban native dragging is scoped to Database/View placement. It never serializes an editor subtree or rewrites a target Card body; moving content between Cards uses the editor's stable-ID `Move to Card` command and atomic cross-Document relocation.
...
- Toggle List summary rows do not export or accept body snapshots; only an independently mounted Card editor can move its own stable-ID Blocks through the atomic relocation protocol.
