...
- Native block drag from visible NFM editors into Kanban columns creates Card(s) using move semantics (source Blocks are removed). Database View result rows are read projections and are not draggable host Blocks.
- Dragging a Kanban Card into a visible NFM editor creates a standalone nested Card/reference shell through the typed drop path and removes the source Card when move semantics are selected.
...
- Toggle List row expansion is window-local and never enters a Card body. Toggles inside an expanded Card's own NFM editor still honor `▼` / `▶` and persist through that Card's Y.Doc.
...
- Migrated inline rules persist losslessly in the durable Database View config. Project-scoped reads validate and execute filter/sort/include-host semantics over memberships, use view rank plus Card ID as stable tie-breakers, and safely show all rows when a malformed legacy rule cannot be interpreted.
...
- Card Stage retains its existing ordered drag sessions with Kanban. Toggle List summary rows do not export or accept body snapshots; only an independently mounted Card editor can move its own stable-ID Blocks through the atomic relocation protocol.
...
- Drag-handle block menu includes a real `Move to` destination popover with grouped DB/card search results. In a primary Card, choosing another Card sends stable root Block IDs plus a logical destination: all mounted source/target editors commit IME, durably flush, and freeze briefly; source deletion, target insertion, registry locations, projections, and history evidence then commit atomically. No renderer removes the source optimistically or reads either Card's NFM snapshot. Legacy Cards retain the grouped snapshot path only until their one-way cutover; DB-column promotion remains a separate create-Card action.
