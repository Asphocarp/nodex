...
- Session panel tabs support `db_view`, `page_stage`, `terminal`, `browser`, `review`, and `files`. Page tabs persist Page identity plus the Project access context used for authorization; Project never becomes the Page owner.
...
- In Electron, startup opens into a blocking bootstrap surface until native Core readiness completes. Fresh Profiles are created as exact Rust-owned v87. Exact published TypeScript v26, v57, v68, v82, and v83 Profiles are backed up and converted in isolation to the exact v84 handoff; direct v84 and Rust-owned v85/v86 Profiles are also accepted. Core validates the complete inventory and content before one-way v87 publication. Unpublished same-version lineages, drifted, ambiguous, future, or damaged stores fail closed while HTTP, schedulers, and windows remain unavailable.
