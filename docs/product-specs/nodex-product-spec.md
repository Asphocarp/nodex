...
- Attached root-thread sessions use a fixed body-portal composer overlay at the bottom of the full-width right panel for `review`, `browser`, `db_view`, and `page_stage` tabs. Project Scenes use the same footer-only connector as a controlled Agent Dock: it renders one explicit task target, latest-turn/request state, and the shared composer without mounting a transcript body. The target can be any exact Session in the Project or a Scene-local `New task`; selection never navigates, and only `Open task` enters the complete Session page. Switching targets replaces the unbound Project/run-target/branch strip or connected latest-turn tray in the same render without a vertical entrance/exit transition, and each target owns its own collapsed latest-turn state. The overlay reuses normal follow-up composer state and actions while presenting an empty draft as a 44px single-line row joined to a collapsible latest-turn tray. Its task-target row, unbound Project/run-target/branch controls, and latest-turn tray share one centered 13px horizontal inset relative to the composer surface. A multiline draft expands into a full-width prompt row above the controls; only the capped prompt editor scrolls. Browser can contract to a 384px idle/hover surface and owns document-bottom auto-hide independently of the Project Dock's persisted visibility. Hiding the Dock releases its bottom reserve and presented-request claim but neither stops the turn nor releases Browser/Terminal runtime; its accessible reveal handle can show activity or request attention. The portal tracks pane geometry, app zoom, and bottom-panel offset. Side chats, Terminal, Files, blank Session homes, and resuming attached Session pages do not show the Session root-thread overlay.
