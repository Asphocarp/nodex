...
- The text-selection menu `More` button closes the text-selection menu and opens the existing block side-menu actions for the currently selected block range. Partial text selections are promoted to block-scoped side-menu actions over every selected block.
