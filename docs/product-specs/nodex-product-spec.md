...
- Automations is a Workbench-owned full-window route opened from the sidebar, command palette, and floating summary Scheduled row. It manages the local `codex_scheduled_automations` read model with list, create, edit, and delete behavior. The route keeps selection in `/automations` search params (`tab`, `automationId`, `automationMode`); creating a task selects the saved automation, editing updates the same row, and deleting the selected task returns to the scheduled-task list.
