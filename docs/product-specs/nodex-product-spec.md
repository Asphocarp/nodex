...
`get_context`, `search`, and `fetch` are eager; both query tools and every write are deferred. Ordinary Turns are evaluated against the Project's bound Database plus recursive Page/Database grants inside its Library. A read-write Page grant allows content edits; structural writes remain bound to the Project Database except at the exact root of an explicitly granted Page. Moving or creating a top-level Library Page establishes a recursive read-write grant for the executing Project, so authority does not disappear when the Page leaves its bound Data Source. A visible ordinary Project task with no task grant reports `write: "consent_required"` and presents the consent card in the active task viewer even when main, another renderer, or no renderer owns canonical conversation state. The card is local to that viewer and survives canonical snapshot refreshes; hiding the view does not discard an existing task grant, while a headless task without one reports `write: "unavailable"`. A Turn launched with the built-in Full access preset instead receives temporary same-Library authority over every capability in the existing catalog, including cross-owner create/move/duplicate and destructive writes, without creating grants or approval cards. Ordinary reads return no concurrency validators. A bounded typed `prepareFor` list asks only for the short title/body/Block ETags required by the next semantic write. Every write still performs mutation-free semantic preflight, exact-Turn and store-epoch validation, reprepare, footprint equality, ETag/CAS checks, and one atomic transaction. Full access removes the Nodex prompt, not these guards.
