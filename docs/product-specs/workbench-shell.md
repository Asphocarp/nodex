...
- The sticky DB toolbar contains the view selector as a horizontal top-edge tab strip plus a Notion-like trailing action cluster; task search expands inline inside that right-side cluster for searchable DB views.
...
- For `calendar`, a compact primary month/year context label sits beside the active Calendar selector, and the trailing cluster replaces unsupported search/filter/sort/display buttons with calendar-local controls: create, range selector, and previous/today/next navigation.
