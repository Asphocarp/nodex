...
- Active session header: uses the active thread title row as the session header, without a separate session title, project-name subtitle, or empty toolbar row above it in either regular or narrow layouts. The fixed global header owns the left chrome (`Sidebar`, `Back`, `Forward`, and collapsed-only `New chat`) plus the top-right `Toggle bottom panel` and `Toggle side panel` controls, ordered bottom first and side second. Those persistent top-right controls stay above and clickable over regular and full-width right panels. Right-panel tab creation and expand/restore actions belong to the right-panel tab header; bottom-panel tab creation plus the plain `Close` X action belong to the bottom-panel tab header. There is no attach/detach thread toolbar button.
...
- Review is a session singleton tab kind across both right and bottom panels. Browser is deliberately multi-tab; creation affordances keep showing Browser even when Browser tabs already exist, and Browser tab context menus add `New tab to the right`, `Reload`, and `Duplicate` before the generic close/move/split actions.
