...
- The block side menu shows live `Header row` and `Header column` actions for a single selected table block. Development/Storybook can additionally show disabled parity rows such as `Fit table width`, row/column color placeholders, and `Create cards from rows`.
