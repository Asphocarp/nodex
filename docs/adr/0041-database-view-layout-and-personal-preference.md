...
The same View supports exactly two layouts, Board and List; layout is not View identity.
Filter and manual order are durable View authority.
Presentation choices such as layout, grouping, subgrouping, sorting, completion visibility, empty groups, and displayed fields may be stored as a sparse Profile-local override keyed by View ID.
...
Personal display changes do not alter the shared query until explicitly published.
...
Store schema v112 migrates durable Views to Config v3 and `default_layout` and adds membership completion timestamps; v113 migrates manual positions to one `(view_id, page_id)` rank.
...
Workbench Scene v6 and Session View v4 persist only durable View identity; layout comes from the effective presentation and is never copied into a tab descriptor.
