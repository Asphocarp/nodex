...
import {
  formatBoundedWorktreeOutput,
  WORKTREE_OUTPUT_TRUNCATION_MARKER,
} from "./worktree-output";
...
describe("formatBoundedWorktreeOutput", () => {
  test("adds the truncation marker outside the retained tail", () => {
    ...
    expect(formatBoundedWorktreeOutput({ text: "complete", didTruncate: false })).toBe(
      "complete",
    );
  });
});
