...
import { parseCodexSessionIndexEntryLine, parseCodexSessionJsonlLine } from "./codex-session";

describe("codex-session schemas", () => {
  test("parses session index lines with codex field aliases", () => {
    const entry = parseCodexSessionIndexEntryLine(
      JSON.stringify({
        id: "thr_1",
  ...
  test("parses jsonl session lines and falls back when payload is not an object", () => {
    const line = parseCodexSessionJsonlLine(
      JSON.stringify({
