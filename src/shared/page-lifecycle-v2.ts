...
const readId = (
  ...
  throw new PageLifecycleV2ContractError(
    `${label}.${key} must be a canonical non-empty identity`,
  );
};
...
const readRecurrence = (value: unknown): RecurrenceConfig | null => {
  ...
  if (
    ...
    (recurrence.interval as number) < 1
  ) {
    throw new PageLifecycleV2ContractError(
      `${label}.interval must be an integer >= 1`,
    );
  }
  ...
  if (recurrence.byWeekdays !== undefined) {
    if (
      ...
      throw new PageLifecycleV2ContractError(
        `${label}.byWeekdays must contain weekdays 0-6`,
      );
    }
    ...
    if (byWeekdays.length !== recurrence.byWeekdays.length) {
      throw new PageLifecycleV2ContractError(
        `${label}.byWeekdays must be unique`,
      );
    }
  }
  ...
  if (recurrence.endCondition !== undefined) {
    ...
    if (condition.type === "never") {
      ...
      throw new PageLifecycleV2ContractError(
        `${label}.endCondition.type is invalid`,
      );
    }
  }
  return {
...
const readActor = (
  ...
  if (canonical.length > MAX_ACTOR_JSON_LENGTH) {
    throw new PageLifecycleV2ContractError(
      "pageLifecycleV2.actor exceeds the JSON size limit",
    );
  }
...
const parseTagOptionIds = (value: unknown): readonly DataSourceOptionId[] => {
  ...
  const ids = value.map((candidate, index) =>
    parseTagOptionId(
      candidate,
      `pageLifecycleV2.operation.tagOptionIds[${index}]`,
    ),
  );
  if (new Set(ids).size !== ids.length) {
    throw new PageLifecycleV2ContractError(
      "pageLifecycleV2.operation.tagOptionIds must be unique",
    );
  }
...
const parseNewTagOptions = (
  ...
  const options = value.map((candidate, index) => {
    ...
    if (!selectedIds.has(optionId)) {
      throw new PageLifecycleV2ContractError(
        `${label}.optionId must also appear in tagOptionIds`,
      );
    }
    return {
      optionId,
  ...
  if (
    new Set(options.map((option) => option.name)).size !==
    options.length
  ) {
    throw new PageLifecycleV2ContractError(
...
const parseCreateOperationV2 = (
  ...
  if (operation.kind !== "create_page") {
    throw new PageLifecycleV2ContractError(
      "pageLifecycleV2.operation.kind must be create_page",
    );
  }
  ...
  const newTagOptions = parseNewTagOptions(
    operation.newTagOptions,
...
const parseOperationV2 = (
  ...
  if (
    ...
    return {
      kind: operation.kind,
      pageId: readId(operation, "pageId", label),
      expectedMetadataRevision: readPositiveRevision(
        operation,
        "expectedMetadataRevision",
        label,
      ),
  ...
  if (operation.kind === "delete_page") {
    ...
    return {
      ...
      expectedParentRevision: readPositiveRevision(
        operation,
        "expectedParentRevision",
        label,
      ),
  ...
  if (operation.kind === "restore_page") {
    ...
    if (operation.membership === null) {
      ...
      if (!isWorkflowStatus(candidate.status)) {
        throw new PageLifecycleV2ContractError(
          `${label}.membership.status is invalid`,
        );
      }
      let dataSourceId: DataSourceId;
      try {
      ...
      let position: Exclude<
        RestorePageOperationV2["membership"],
        null
      >["position"];
  ...
  throw new PageLifecycleV2ContractError(
    "pageLifecycleV2.operation.kind is not supported",
  );
};
...
const canonicalizeParsedRequest = (
...
export const canonicalizePageLifecycleMutationRequestV2 = (
  value: unknown,
...
const PAGE_LIFECYCLE_OPERATION_KINDS_V2 = new Set<
  PageLifecycleOperationV2["kind"]
>([
  "create_page",
  "archive_page",
  "unarchive_page",
...
export const parsePageLifecycleMutationReceiptV2 = (
  ...
  if (
    ...
    throw new PageLifecycleV2ContractError(
      `${label}.operationKind is invalid`,
    );
  }
  if (
  ...
  if (
    ...
    throw new PageLifecycleV2ContractError(
      `${label}.createdBlockIds is invalid`,
    );
  }
  ...
  return {
    ...
    metadataRevision: readPositiveRevision(
      receipt,
      "metadataRevision",
      label,
    ),
...
export const parsePageLifecycleMutationCommandErrorV2 = (
  ...
  if (
    ...
    !PAGE_LIFECYCLE_V2_ERROR_CODES.has(
      error.code as PageLifecycleMutationErrorCodeV2,
    )
  ) {
    throw new PageLifecycleV2ContractError(
      "pageLifecycleErrorV2.code is invalid",
    );
  }
  if (
  ...
  if (typeof error.retryable !== "boolean") {
    throw new PageLifecycleV2ContractError(
      "pageLifecycleErrorV2.retryable must be a boolean",
    );
  }
  const operationId = error.operationId === undefined
    ? undefined
    : readId(error, "operationId", "pageLifecycleErrorV2");
  const pageId = error.pageId === undefined
    ? undefined
    : readId(error, "pageId", "pageLifecycleErrorV2");
  const expectedRevision = error.expectedRevision === undefined
    ? undefined
    : readNonNegativeRevision(
        error,
        "expectedRevision",
        "pageLifecycleErrorV2",
      );
  ...
  if (
    (expectedRevision === undefined) !== (actualRevision === undefined)
  ) {
    throw new PageLifecycleV2ContractError(
...
export const parsePageLifecycleMutationCommandResultV2 = (
  ...
  throw new PageLifecycleV2ContractError(
    "pageLifecycleResultV2.ok must be a boolean",
  );
};
