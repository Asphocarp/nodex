...
export function parseCodexFileChangePatchUpdate(
  method: string,
  params: unknown,
): CodexFileChangePatchUpdate | null {
  if (method !== "item/fileChange/patchUpdated") return null;
...
export function parseCodexMcpToolCallProgressUpdate(
  method: string,
  params: unknown,
): CodexMcpToolCallProgressUpdate | null {
  if (method !== "item/mcpToolCall/progress") return null;
  const payload = asRecord(params);
  if (
    !payload
    || typeof payload.threadId !== "string"
