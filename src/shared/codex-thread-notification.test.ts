...
describe("codex thread notification policy", () => {
  test("only parent provenance classifies a child conversation", () => {
    ...
    expect(isCodexNotificationChildConversation(conversation({
  ...
  test("does not blanket suppress system, ephemeral-like, or side conversations", () => {
    ...
    expect(decideCodexTurnNotification(turn({
      conversation: conversation({ threadSource: "system" }),
    }), policy)).toEqual({ type: "show" });
    expect(decideCodexTurnNotification(turn({
      conversation: conversation({
  ...
  test("applies turn suppression in reference order for every terminal status", () => {
    ...
    }), always)).toEqual({ type: "suppress", reason: "automation-dont-notify" });
    expect(decideCodexTurnNotification(turn({
    ...
    }), always)).toEqual({ type: "suppress", reason: "heartbeat-dont-notify" });
    expect(decideCodexTurnNotification(turn({
      conversation: conversation({ parentThreadId: "parent" }),
    }), always)).toEqual({ type: "suppress", reason: "child-conversation" });
    expect(decideCodexTurnNotification(turn({
      conversation: conversation({ threadSource: "realtime_voice" }),
    }), always)).toEqual({ type: "suppress", reason: "realtime-voice" });
    expect(decideCodexTurnNotification(turn({
      hasPendingContinuation: true,
    }), always)).toEqual({ type: "suppress", reason: "pending-continuation" });
    expect(decideCodexTurnNotification(turn(), {
      ...always,
  ...
  test("suppresses requests only for settings, real children, or foreground presentation", () => {
    expect(decideCodexRequestNotification(conversation(), {
      enabled: true,
  ...
  test("uses heartbeat copy precedence and exact request copy", () => {
    expect(resolveCodexTurnNotificationBody(turn({
    ...
    expect(resolveCodexTurnNotificationBody(turn({ lastAgentMessage: null })))
      .toBe("Nodex finished a turn.");
    expect(resolveCodexApprovalNotificationCopy({
  ...
  test("builds host-qualified public IDs while preserving strict IDs internally", () => {
    ...
    expect(buildCodexApprovalNotificationId("remote-a", "73")).toBe("approval-remote-a-73");
    expect(buildCodexQuestionNotificationId("default", "request-1"))
      .toBe("question-default-request-1");
    expect(buildCodexRequestNotificationOccurrenceId(
      "approval",
      "remote-a",
      "thread-1",
      73,
    )).not.toBe(buildCodexRequestNotificationOccurrenceId(
      "approval",
      "remote-a",
      "thread-1",
      "73",
