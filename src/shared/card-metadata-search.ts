...
export type CardMetadataMatchQuality = "exact" | "prefix" | "fuzzy";

export interface CardMetadataSearchProperty {
  readonly propertyId: string;
  readonly propertyName: string;
  readonly text: string;
}

export interface CardMetadataSearchDocument {
  readonly id: string;
  readonly identity: string;
  readonly title: string;
  readonly properties: readonly CardMetadataSearchProperty[];
}

export interface CardMetadataSearchEvidence {
  readonly term: string;
  readonly source: "identity" | "title" | "property";
  readonly quality: CardMetadataMatchQuality;
  readonly excerpt: string;
  readonly propertyId?: string;
  readonly propertyName?: string;
}

export interface CardMetadataSearchHit {
  readonly id: string;
  readonly matchedTerms: readonly string[];
  readonly evidence: readonly CardMetadataSearchEvidence[];
...
function fieldQuality(text: string, term: string): CardMetadataMatchQuality | null {
...
function evidenceForResult(
  source: CardMetadataSearchDocument,
  term: string,
  result: SearchResult,
): CardMetadataSearchEvidence[] {
  const fields = matchedFields(result);
  const evidence: CardMetadataSearchEvidence[] = [];
...
export function searchCardMetadata(
  documents: readonly CardMetadataSearchDocument[],
  query: string,
): readonly CardMetadataSearchHit[] {
  ...
  const hits = new Map<string, {
    evidence: CardMetadataSearchEvidence[];
