  ...
  return (
    <div>
      ...
      <output data-testid="state">
        {JSON.stringify({ routeState, preferences })}
      </output>
    </div>
  );
}
...
describe("Review view state", () => {
  test("canonicalizes absolute and patch-prefixed paths and resolves rename aliases", () => {
    expect(canonicalizeReviewPath("/workspace/./src/app.ts", ["/workspace"])).toBe(
      "src/app.ts",
    );
    expect(canonicalizeReviewPath("b/src/app.ts", ["/workspace"])).toBe(
      "src/app.ts",
    );
    const candidates = [
      { displayPath: "src/new.ts", previousPath: "src/old.ts" },
    ];
    expect(
      resolveReviewPathCandidate(
        candidates,
        canonicalizeReviewPath("a/src/old.ts"),
      )?.displayPath,
    ).toBe("src/new.ts");
  });
  ...
  test("fails closed when an alias resolves to more than one Review file", () => {
    ...
    expect(resolveReviewPathCandidate([
  ...
  test("prefers an exact current path over a rename alias", () => {
    ...
    ], target)?.displayPath).toBe("src/current.ts");
  });
