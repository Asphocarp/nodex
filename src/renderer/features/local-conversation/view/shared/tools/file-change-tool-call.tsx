...
function resolveOpenPath(path: string | null, basePath: string | null): string | null {
  if (!path) return null;
...
function buildRowModelFromFileDiff(
  ...
  return {
    ...
    label: labels.label,
    expandedLabel: labels.expandedLabel,
    summary: summarizeDiff(undefined),
    fileDiff,
    openLine: resolveOpenLine(fileDiff),
  };
}
...
function DiffStats({ additions, deletions }: DiffSummary) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="inline-flex items-center gap-1 disambiguated-digits tabular-nums tracking-tight text-size-chat-sm">
