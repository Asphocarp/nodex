...
export function deriveComposerIntelligenceSelection(
  ...
  if (model.agentProviderCatalog && model.executionProfile) {
    return {
      kind: "agent",
      profile: model.isNewThreadTab && model.executionProfile.serviceTier === null
  ...
  return {
    ...
    serviceTier:
      model.conversation?.latestThreadSettings?.serviceTier
      ?? defaultServiceTier,
  };
}
...
export function areComposerIntelligenceSelectionsEqual(
  ...
  return left.profile.providerId === right.profile.providerId
