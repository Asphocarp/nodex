...
export type ComposerSuggestionKind =
  | "at-mention"
  | "skill-mention"
  | "slash-command";
...
export const composerSuggestionPluginKey =
  new PluginKey<ComposerSuggestionState>("composer-suggestion");
...
function deriveTypedSuggestionState(input: {
  ...
  const slashMatch =
    /(?:^|\s)\/([\p{L}\p{N}\p{M}.:_/\\-]*)$/u.exec(beforeCursor.text);
  ...
  let match: {
    ...
    ? { trigger: "/", query: slashMatch[1] ?? "" }
    : null;
  if (
    atMatch
    && (!skillMatch || (atMatch.index ?? 0) > (skillMatch.index ?? 0))
  ) {
  ...
  return {
    ...
    query,
    range: { from, to },
    source: input.previous?.active === true
      && input.previous.range?.from === from
...
function deriveSyntheticSuggestionState(input: {
  ...
  if (
    typed.active
    && typed.range !== null
    && typed.range.from >= from
  ) {
    return typed;
  }

  const to = input.previous.kind === "slash-command"
    ? mappedTo
    : selection.from;
  if (to < from) return inactiveComposerSuggestionState();

  const query = input.transaction.doc.textBetween(
    from,
    to,
    "\n",
    "\n",
  ...
  return {
    ...
    query,
    range: { from, to },
    source: input.previous.kind === "slash-command"
      ? input.previous.source
      : null,
  };
}
...
function applyComposerSuggestionTransaction(
  ...
  ) as ComposerSuggestionTransactionMeta | undefined;
...
export function readComposerSuggestionState(
  state: EditorState,
): ComposerSuggestionState {
  return composerSuggestionPluginKey.getState(state)
