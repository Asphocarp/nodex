...
const EMPTY_SECTION_LIMITS: Partial<
  Record<ComposerContextSuggestionSection, number>
> = {
  Apps: 3,
...
function scoreCandidate(
  ...
  return Math.max(
    ...
    ...candidate.searchTerms.map((term) =>
      scoreFuzzyQueryMatch(term, query)
    ),
  );
}
...
function resolveSearchPriority(
  ...
  ].some((term) => normalizeSearchText(term).startsWith(query));
  if (startsWithQuery) {
  ...
  return input.candidates
    ...
    .sort((left, right) => {
      if (input.useProviderPriority !== false) {
        const leftPriority = resolveSearchPriority(
          left.candidate,
          normalizedQuery,
        );
        const rightPriority = resolveSearchPriority(
          right.candidate,
          normalizedQuery,
        );
      ...
      if (input.tieBreakByLabel) {
        const labelOrder = left.candidate.label.localeCompare(
...
export function buildComposerContextSuggestionSections<T>(input: {
  ...
  readonly loadingSectionMessages?: Partial<
    Record<ComposerContextSuggestionSection, string>
  >;
  ...
  return sectionOrder.flatMap((section) => {
    const items = input.candidates.filter((candidate) =>
