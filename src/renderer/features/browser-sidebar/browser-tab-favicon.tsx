...
export interface BrowserTabFaviconStateInput extends Omit<
  BrowserTabFaviconProps,
  "className"
> {
  reduceMotion: boolean;
}
...
export function reconcileBrowserTabFaviconState(
  ...
  const failedFaviconUrl = previous.faviconUrl === input.faviconUrl
    ? previous.failedFaviconUrl
    : null;
...
export function BrowserTabFavicon({
  ...
  const handleFaviconLoadError = (failedUrl: string) => {
    setState((current) => {
      if (
        current.faviconUrl !== failedUrl
        && current.pinnedCompletionFaviconUrl !== failedUrl
      ) {
        return current;
      }
      ...
      return {
        ...
        pinnedCompletionFaviconUrl: failedPinnedFavicon
          ? null
          : current.pinnedCompletionFaviconUrl,
        skipCompletionTransition:
          failedPinnedFavicon
          || (!current.isLoading && current.faviconUrl === failedUrl)
            ? true
...
export function BrowserTabFaviconFrame({
  ...
  const currentFailedFaviconUrl = failedFaviconUrl === faviconUrl
    ? failedFaviconUrl
    : null;

  const isLoadingPhase = phase === "spinner-only" || phase === "loading-favicon";
  const faviconCanLoad =
    faviconUrl != null && currentFailedFaviconUrl !== faviconUrl;
  const showFavicon =
    phase !== "spinner-only"
    && (phase !== "loading-favicon" || faviconCanLoad);
  const disableTransition =
    skipCompletionTransition
    || reduceMotion
    || (phase === "settled" && !faviconCanLoad);
      ...
      <span
        className={cn(
          ...
          phase === "completion-start-frame"
            && "nodex-browser-tab-favicon-clip-completion-start",
          phase === "completion-midpoint-frame"
            && "nodex-browser-tab-favicon-clip-completion-midpoint",
        )}
        data-browser-tab-favicon-clip="true"
        onTransitionCancel={onCompletionTransitionFinish == null
          ? undefined
          : handleCompletionTransition}
        onTransitionEnd={onCompletionTransitionFinish == null
          ? undefined
          : handleCompletionTransition}
      >
        <span
