...
export interface PortableRichTitleProps extends Omit<
...
const renderItem = (item: PortableRichTextItem, index: number): ReactNode => {
  ...
  return (
    <span
      key={`${item.type}:${index}`}
      data-portable-rich-title-link={
        item.type === "link" ? item.href : undefined
      }
...
export function PortableRichTitle({
  ...
  return (
    <span
      {...props}
      className={cn("min-w-0 whitespace-pre-wrap wrap-break-word", className)}
    >
