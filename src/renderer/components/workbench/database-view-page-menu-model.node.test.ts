...
describe("Database View Page menu model", () => {
  test("builds the shared Page hierarchy in product order", () => {
    ...
    expect(actions.find((action) => action.id === "move")?.children
      ?.map((action) => [action.label, action.disabled])).toEqual([
        ["Move to top", true],
        ["Move up", true],
        ["Move down", false],
        ["Move to bottom", false],
      ]);
    expect(actions.find((action) => action.id === "copy")?.children
    ...
    expect(actions.find((action) => action.id === "open-in")?.children
      ?.map((action) => action.label)).toEqual([
        "Open in new session",
        "Send to chat…",
      ]);
  ...
  test("does not invent an ID when a Page has no current key", () => {
    ...
    expect(actions.find((action) => action.id === "copy")?.children
      ?.some((action) => action.id === "copy-id")).toBe(false);
  });
  ...
  test("keeps a parent trigger and only matching descendants during search", () => {
    ...
    expect(actions[0]?.children?.map((action) => action.id)).toEqual([
      "copy-markdown",
    ]);
  });
