...
export const WORKBENCH_AUTOMATION_FIRST_RUN_SUGGESTIONS: readonly WorkbenchAutomationFirstRunSuggestion[] =
  [
    ...
    },
    {
      ...
      prompt:
        "I want to set up a project monitor scheduled task. Briefly explain how scheduled tasks work in Nodex, then ask me what project to watch, what changes matter, and when it should check in",
    },
  ];
...
export function filterWorkbenchAutomationTemplates(
  ...
  return templates.filter((template) =>
    normalizeTemplateSearchText(
      [template.name, template.prompt, template.scheduleLabel].join(" "),
    ).includes(query),
  );
}
