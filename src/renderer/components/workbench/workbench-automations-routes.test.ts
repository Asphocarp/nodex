...
describe("workbench automations routes", () => {
  ...
  test("preserves Codex automationId and automationMode search semantics", () => {
    expect(buildAutomationsPath({
      automationId: "automation-1",
      automationMode: "create",
    })).toBe("/automations?automationId=automation-1&automationMode=create");

    expect(JSON.stringify(resolveAutomationsRouteState("/automations?tab=templates&automationId=automation-2&automationMode=create")))
      .toBe(JSON.stringify({
        tab: "templates",
  ...
  test("updates selection while preserving current tab by default", () => {
    expect(updateAutomationsPath("/automations?tab=templates", {
      automationId: "automation-3",
    })).toBe("/automations?tab=templates&automationId=automation-3");

    expect(updateAutomationsPath("/automations?automationId=automation-3&automationMode=create", {
      automationId: null,
      automationMode: null,
    })).toBe("/automations");
  });
