...
describe("NFM heading navigation rail model", () => {
  test("collects nested heading blocks with normalized levels and fallback labels", () => {
    ...
    expect(JSON.stringify(headings.map((heading) => `${heading.id}:${heading.ordinal}:${heading.level}:${heading.depth}:${heading.label}`))).toBe(
      JSON.stringify(["a:1:1:0:Intro", "b:2:3:2:Nested", "c:3:4:3:Untitled"]),
    );
  });
