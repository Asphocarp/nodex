...
export const taskShorthandPreviewPluginKey =
  new PluginKey<TaskShorthandDecorationState>("task-shorthand-preview");
...
const blockDecoration = (
  ...
  node.forEach((child, offset) => {
    ...
    const crossesRichAuthority = !child.isText
      || child.marks.some((mark) => mark.type.name === "link");
    if (crossesRichAuthority) firstAuthorityBoundary = offset;
  });
  if (
    firstAuthorityBoundary !== null
    && preview.consumedCharacters > firstAuthorityBoundary
  ) return null;
  ...
  return Decoration.inline(
    ...
    {
      class: "rounded-[2px] bg-token-accent/8 underline decoration-token-accent/45 decoration-dotted underline-offset-[3px]",
...
const updateChangedBlockDecorations = (
  ...
  for (const position of positions) {
    ...
    decorations = decorations.remove(
      decorations.find(position, position + node.nodeSize),
    );
    const decoration = blockDecoration(node, position);
...
const createTaskShorthandPreviewPlugin = () => new Plugin<TaskShorthandDecorationState>({
  key: taskShorthandPreviewPluginKey,
  state: {
    init: (_, state) => ({ decorations: buildDecorations(state.doc) }),
    apply: (transaction, previous) => transaction.getMeta(taskShorthandPreviewPluginKey) === "refresh"
  ...
  props: {
    decorations: (state) => taskShorthandPreviewPluginKey.getState(state)?.decorations
      ?? DecorationSet.empty,
  },
  view: (view) => {
