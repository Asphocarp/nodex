...
function createFakeEditor() {
  ...
  const editor: TextActionEditorAdapter = {
    ...
    updateBlock: (block, update) => {
      const blockId = typeof block === "object" && block !== null && "id" in block
        ? String(block.id)
        : "unknown";
    ...
    addStyles: (styles) => {
      calls.push(`addStyles:${Object.entries(styles).map(([key, value]) => `${key}=${value}`).join(",")}`);
    },
    removeStyles: (styles) => {
...
describe("nfm text action menu actions", () => {
  test("converts selected blocks inside the editor transaction boundary", () => {
    ...
    const didApply = applyTextActionBlockType(
      editor,
      [{ id: "a" }, { id: "b" }],
  ...
  test("builds and applies schema-filtered clear-format operations", () => {
    ...
    const styles = buildTextActionClearFormatStyles(
      editor,
