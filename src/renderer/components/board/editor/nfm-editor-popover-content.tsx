...
function composePreventDefaultAutoFocus(handler: AutoFocusHandler | undefined): AutoFocusHandler {
  return (event) => {
    handler?.(event);
...
export const NfmEditorPopoverContent = forwardRef<HTMLDivElement, NfmEditorPopoverContentProps>(
  function NfmEditorPopoverContent({ onOpenAutoFocus, onCloseAutoFocus, ...props }, ref) {
    return (
      <NodexPopoverContent
        ref={ref}
