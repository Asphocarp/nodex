...
export function usePageStageProperties(input: {
  ...
  const properties = databaseContext?.kind === "member"
    ? databaseContext.properties
    : EMPTY_PROPERTIES;
  const semantic = databaseContext?.kind === "member"
    ? databaseContext.semanticProperties
    : null;
  ...
  useEffect(() => {
    const active = new Set<string>(
      properties.map((item) => item.property.propertyId),
    );
    setPendingPropertyCounts((current) => new Map(
      [...current].filter(([propertyId]) => active.has(propertyId)),
    ));
    setErrors((current) => Object.fromEntries(
      Object.entries(current).filter(([propertyId]) => active.has(propertyId)),
    ));
  }, [properties]);
  ...
  ) => edit(property, { kind: "patch_multi_select", ...delta }), [edit]);
  ...
  const requestOptions = useCallback(
    (property: PageStageDataSourceProperty) =>
      optionRegistries.requestOptions(property.property),
    [optionRegistries],
  );
  ...
  return useMemo(() => ({
    ...
    sectionProperties: semantic
      ? pageStageSectionProperties(properties, semantic)
      : properties,
    semanticValues: semantic ? pageStageSemanticValues(semantic) : null,
    hasScheduleCapability: semantic
      ? hasPageStageScheduleCapability(semantic)
      : false,
    options,
