...
type BoardCardDisplayProperty = Extract<DbViewDisplayPropertyKey, "priority" | "estimate" | "tags" | "assignee">;
type CardType = DatabasePageSummary;

const DEFAULT_BOARD_CARD_DISPLAY_ORDER: BoardCardDisplayProperty[] = [
  "priority",
  "estimate",
  "tags",
  "assignee",
...
function CardPropertyBadges({
  ...
  const propertyOrder = (displayPrefs?.propertyOrder ?? DEFAULT_BOARD_CARD_DISPLAY_ORDER)
    .filter(
      (property): property is BoardCardDisplayProperty =>
        DEFAULT_BOARD_CARD_DISPLAY_ORDER.includes(property as BoardCardDisplayProperty),
    );
  ...
  const renderProperty = (property: BoardCardDisplayProperty) => {
...
const ResolvedCardBody = memo(function ResolvedCardBody({
  ...
  const draftResolvedCard = useMemo(
    () => mergePageDraftOverlay(card, draftOverlay) ?? card,
    [card, draftOverlay],
  );
  const presentedTitle = usePresentedPageTitle(
    card.id,
    draftResolvedCard.title,
  );
  const resolvedCard = useMemo(
    () => presentedTitle === draftResolvedCard.title
      ? draftResolvedCard
      : { ...draftResolvedCard, title: presentedTitle },
    [draftResolvedCard, presentedTitle],
  );
  ...
  useEffect(() => {
    ...
    const draggableCleanup = draggable({
      ...
      onGenerateDragPreview: ({ location, nativeSetDragImage, source }) => {
        const dragData = isBoardCardDragData(source.data)
          ? source.data
    ...
    return combine(
      ...
      dropTargetForElements({
        element,
        canDrop: ({ source }) => canDropOnBoardCard({
          targetPageId: card.id,
          source: source.data,
          instanceId: dragInstanceId,
        }),
        getIsSticky: () => true,
        getData: () => buildBoardCardDropTargetData({
          instanceId: dragInstanceId,
          pageId: card.id,
