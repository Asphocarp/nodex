...
describe("Due date value icon", () => {
  test("distinguishes due-now and future calendar visuals by local date", () => {
    ...
    expect(databaseDueDateIconColor("2026-08-11", today)).toBe(
      DATABASE_DUE_DATE_NOW_ICON_COLOR,
    );
    expect(databaseDueDateIconColor("2026-08-12", today)).toBe(
      DATABASE_DUE_DATE_NOW_ICON_COLOR,
    );
    expect(databaseDueDateIconColor("2026-08-13", today)).toBe(
      DATABASE_DUE_DATE_FUTURE_ICON_COLOR,
    );
    expect(databaseDueDateIconColor("not-a-date", today)).toBe(
      DATABASE_DUE_DATE_MUTED_ICON_COLOR,
    );
  });
