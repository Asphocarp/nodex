...
const property = (
  ...
  valueType,
  config: { options: [{ id: "a", name: "A" }, { id: "b", name: "B" }] },
  rankKey: propertyId,
  lifecycle: "active",
  revision: 1,
  createdAt: timestamp,
...
describe("database task filter adapter", () => {
  ...
  test("declines filters that require the generic editor", () => {
    expect(decodeDatabaseTaskFilter({
      kind: "clause",
  ...
  test("keeps an all-deselected choice representable without falling back", () => {
    ...
    expect(decodeDatabaseTaskFilter(encoded, capabilities)?.groups[0]?.status)
      .toEqual({ selectedOptionIds: [], includeEmpty: false });
  });
});
