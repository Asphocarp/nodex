...
const snapshotsByChannelId = new Map<
  ImageEditComposerChannelId,
  ImageEditComposerDraftSnapshot
>();
const draftListenersByChannelId = new Map<
  ImageEditComposerChannelId,
  Set<() => void>
>();
const handlersByChannelId = new Map<
  ImageEditComposerChannelId,
  ImageEditComposerSubmitHandler
>();
...
function publishDraft(
  ...
  snapshotsByChannelId.set(normalizedChannelId, Object.freeze({
    attachments: Object.freeze(input.attachments.map((attachment) => Object.freeze({
      ...attachment,
      asset: Object.freeze({ ...attachment.asset }),
      comments: Object.freeze(attachment.comments.map((comment) => (
...
export function clearImageEditComposerDraft(
  channelId: ImageEditComposerChannelId,
): void {
  const normalizedChannelId = normalizeChannelId(channelId);
...
export function subscribeImageEditComposerDraft(
  ...
  const normalizedChannelId = normalizeChannelId(channelId);
  if (!normalizedChannelId) return () => undefined;
  const listeners = draftListenersByChannelId.get(normalizedChannelId)
    ?? new Set<() => void>();
...
export function compileImageEditComposerPrompt(args: {
  ...
  const commented = args.draft.attachments.filter(
    (attachment) => attachment.comments.length > 0,
  );
