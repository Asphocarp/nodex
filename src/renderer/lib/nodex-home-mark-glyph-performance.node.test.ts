...
describe("Nodex home mark glyph performance", () => {
  test("uses the approved hello scene at every authored boundary", () => {
    for (const boundary of NODEX_HOME_MARK_HELLO_BOUNDARIES) {
      expect(resolveNodexHomeMarkHelloFrame(
        millisecondsAtFrame(boundary.atFrame),
      ).sceneId).toBe(boundary.sceneId);
    }
    expect(resolveNodexHomeMarkHelloFrame(
      millisecondsAtFrame(NODEX_HOME_MARK_HELLO_DURATION_FRAMES),
    )).toEqual({
      complete: true,
  ...
  test("holds the loader cursor for twenty frames before every hard cut", () => {
    expect(resolveNodexHomeMarkLoaderFrame(millisecondsAtFrame(19.99)).sceneId)
      .toBe("prompt");
    expect(resolveNodexHomeMarkLoaderFrame(millisecondsAtFrame(20)).sceneId)
      .toBe("prompt-no-cursor");
    expect(resolveNodexHomeMarkLoaderFrame(millisecondsAtFrame(40)).sceneId)
      .toBe("prompt");
    expect(resolveNodexHomeMarkLoaderFrame(
      millisecondsAtFrame(NODEX_HOME_MARK_LOADER_DURATION_FRAMES),
    ).complete).toBe(true);
  });
