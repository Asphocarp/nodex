  ...
  Layer.effectDiscard(
    Effect.gen(function* () {
      ...
      yield* projectDocumentCommand(
        ...
        (target, request) =>
          options.documents.publishAwareness(
            { kind: "project", projectId: request.projectId },
            target,
            omitProjectScope(request),
          ),
      ...
      yield* libraryDocumentCommand(
        ...
        (target, request) => options.documents.subscribe({ kind: "library" }, target, request),
      );
      yield* libraryDocumentCommand(
        "library-document-sync:unsubscribe",
        "unsubscribe-library-document",
        (target, request) => options.documents.unsubscribe({ kind: "library" }, target, request),
      );
      yield* libraryDocumentCommand(
        "library-document-sync:sync",
        "sync-library-document",
        (target, request) => options.documents.sync({ kind: "library" }, target, request),
      );
      yield* libraryDocumentCommand(
        "library-document-sync:apply",
        "apply-library-document",
        (target, request) => options.documents.applyUpdate({ kind: "library" }, target, request),
      );
      yield* libraryDocumentCommand(
        "library-document-sync:awareness:publish",
        "publish-library-awareness",
        (target, request) =>
          options.documents.publishAwareness({ kind: "library" }, target, request),
      );
