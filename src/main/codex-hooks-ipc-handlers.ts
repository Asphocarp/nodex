...
export type CodexHooksIpcChannel =
  | "codex:hooks:list"
...
export function registerCodexHooksIpcHandlers(
  options: CodexHooksIpcRegistration,
): void {
  options.registerHandle("codex:hooks:list", (_, input) =>
    options.listHooks(input),
  );
