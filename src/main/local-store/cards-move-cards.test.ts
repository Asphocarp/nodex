...
import { createCard, moveCards } from "./cards";
...
describe("moveCards", () => {
  test("treats same-column move-many newOrder as the post-removal insertion index", async () => {
    const ran = await withTempDatabase(async () => {
      ...
      await createCard(projectId, "in_progress", { title: "Third" });
      const fourth = await createCard(projectId, "in_progress", { title: "Fourth" });
  ...
  test("undo restores original order for same-column grouped moves", async () => {
    const ran = await withTempDatabase(async () => {
      ...
      const third = await createCard(projectId, "in_progress", { title: "Third" });
      await createCard(projectId, "in_progress", { title: "Fourth" });
  ...
  test("moves cards to another column and undoes the group in one step", async () => {
    const ran = await withTempDatabase(async () => {
      const first = await createCard(projectId, "in_progress", { title: "First" });
      const second = await createCard(projectId, "in_progress", { title: "Second" });
      await createCard(projectId, "in_review", { title: "Existing review" });
  ...
  test("moves a cross-column selection into one target column in board order", async () => {
    const ran = await withTempDatabase(async () => {
      const inProgressFirst = await createCard(projectId, "in_progress", { title: "In Progress 1" });
      await createCard(projectId, "in_progress", { title: "In Progress 2" });
      const reviewFirst = await createCard(projectId, "in_review", { title: "Review 1" });
      await createCard(projectId, "in_review", { title: "Review 2" });
