...
const historyRequest = (
  fixture: Fixture,
  cardId: string,
): Pick<ListCardHistoryRequest, "version" | "projectId" | "cardBlockId"> => ({
...
const createCard = (fixture: Fixture, cardId: string): CardFixture => {
  ...
  fixture.database
    .prepare("INSERT INTO blocks (id, project_id, type) VALUES (?, ?, 'card')")
  ...
  insertMutation(fixture, card, {
    ...
    committedAt: T0,
    mutationKind: "card_lifecycle",
    actor: { displayName: "History tester" },
    payload: { operation: "create_card" },
  });
  return card;
};
...
describe("canonical Card history", () => {
  test("merges checkpoints, mutations, and relocations with stable cursors", () => {
    withFixture((fixture) => {
      ...
      const first = listCardHistory(fixture.database, {
      ...
      const second = listCardHistory(fixture.database, {
        ...historyRequest(fixture, card.cardId),
      ...
      const third = listCardHistory(fixture.database, {
        ...historyRequest(fixture, card.cardId),
      ...
      expect(
        third.entries[0]?.kind === "block_relocation" &&
          ...
          third.entries[0].direction === "into_card",
      ).toBe(true);
      expect(third.entries[0]?.evidence.status).toBe("verified");
      expect(third.entries[1]?.display.title).toBe("Created Card");
      expect(third.nextCursor).toBe(null);

      const repeatedSecond = listCardHistory(fixture.database, {
        ...historyRequest(fixture, card.cardId),
        before: first.nextCursor,
        pageSize: 1,
      });
  ...
  test("fails closed across Projects and excludes cross-scope changes", () => {
    withFixture((fixture) => {
      const card = createCard(fixture, "history-scope-card");
      const before = listCardHistory(
        fixture.database,
        historyRequest(fixture, card.cardId),
      );
      ...
      expect(
        errorCode(() =>
          listCardHistory(fixture.database, {
            version: CARD_HISTORY_CONTRACT_VERSION,
            projectId: "project:foreign",
            cardBlockId: card.cardId,
          }),
        ),
      ).toBe("card_not_found");
  ...
  test("bounds display fields and rejects invalid request or cursor budgets", () => {
    withFixture((fixture) => {
      ...
      ).toBe("invalid_card_history_request");
      expect(
        errorCode(() =>
          listCardHistory(fixture.database, {
            ...historyRequest(fixture, card.cardId),
  ...
  test("reads a standalone Card without Database membership", () => {
    withFixture((fixture) => {
      ...
      expect(
        page.entries.every(
          (entry: CardHistoryEntry) => entry.cardBlockId === card.cardId,
        ),
      ).toBe(true);
    });
  });
