...
// getDb uses the shared module-level SQLite singleton.
...
export interface UndoRedoLatestResult {
  success: boolean;
...
export function undoLatest(projectId: string, sessionId?: string): UndoRedoLatestResult {
  const canonicalProjectId = requireProjectId(projectId);
  const target = getUndoTarget(canonicalProjectId, sessionId);
  if (!target) {
    const state = getUndoRedoState(canonicalProjectId, sessionId);
    return { success: false, error: "Nothing to undo", ...state };
  }
  const result = executeUndoWithGrouping(target.id);
  const state = getUndoRedoState(canonicalProjectId, sessionId);
  return {
...
export function redoLatest(projectId: string, sessionId?: string): UndoRedoLatestResult {
  const canonicalProjectId = requireProjectId(projectId);
  const target = getRedoTarget(canonicalProjectId, sessionId);
  if (!target) {
    const state = getUndoRedoState(canonicalProjectId, sessionId);
    return { success: false, error: "Nothing to redo", ...state };
  }
  const result = executeRedoWithGrouping(target.id);
  const state = getUndoRedoState(canonicalProjectId, sessionId);
  return {
    success: result.success,
    entry: result.entry ? { operation: result.entry.operation, cardId: result.entry.cardId } : undefined,
