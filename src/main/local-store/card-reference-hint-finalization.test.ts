...
describe("Card reference identity finalization", () => {
  test("commits hint deletion through the Document writer and is idempotent", async () => {
    useTempStore();
    ...
    expect(finalizeCardReferenceIdentityStorage(database)).toMatchObject({
      updatedDocuments: 1,
      removedHints: 1,
    });
