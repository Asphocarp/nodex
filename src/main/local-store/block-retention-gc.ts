...
export type BlockRetentionGcFaultPoint =
  | "after_ownership_delete"
  | "after_block_delete"
  | "after_document_delete"
  | "before_commit";

export interface CollectBlockRetentionGcInput
  extends PlanBlockRetentionGcInput {
  readonly rootBlockIds: readonly string[];
}

export type CollectBlockRetentionGcResult =
  | {
      readonly status: "collected";
      readonly projectId: string;
      readonly collectedRootBlockIds: readonly string[];
      readonly deletedBlockIds: readonly string[];
      readonly deletedDocumentIds: readonly string[];
    }
  | {
      readonly status: "retained";
      readonly plan: BlockRetentionGcPlan;
    }
  | {
...
export interface CollectBlockRetentionGcOptions {
  readonly faultInjector?: (point: BlockRetentionGcFaultPoint) => void;
}
...
const appendConstraintBlocker = (
  plan: BlockRetentionGcPlan,
): BlockRetentionGcPlan => ({
  ...plan,
  candidates: plan.candidates.map((candidate) => ({
    ...
    blockers: [
      ...candidate.blockers,
      {
        kind: "unclassified_foreign_key_root" as const,
        count: 1,
        samples: [
          {
            source: "sqlite",
            identity: candidate.rootBlockId,
            relation: "delete_constraint_not_classified",
          },
        ],
      },
    ],
...
export const collectBlockRetentionGc = (
  database: Database.Database,
  input: CollectBlockRetentionGcInput,
  options: CollectBlockRetentionGcOptions = {},
): CollectBlockRetentionGcResult => {
  const projectId = requireIdentity(input.projectId, "projectId");
  const requested = uniqueIdentities(
    input.rootBlockIds,
    "rootBlockIds",
    input.policy?.maxCandidates ??
      DEFAULT_BLOCK_RETENTION_GC_POLICY.maxCandidates,
  );
  if (requested.length === 0) {
    throw new TypeError("collectBlockRetentionGc requires a root Block");
  }
  let lastPlan: BlockRetentionGcPlan | null = null;
  try {
    return database
      .transaction((): CollectBlockRetentionGcResult => {
        const plan = planBlockRetentionGc(database, {
          ...input,
          projectId,
          rootBlockIds: requested,
        });
        lastPlan = plan;
        if (plan.candidates.some((candidate) => !candidate.collectible)) {
          return { status: "retained", plan };
        }
        const blockIds = [
          ...new Set(
            plan.candidates.flatMap((candidate) => candidate.closureBlockIds),
          ),
        ].sort();
        const documentIds = [
          ...new Set(
            plan.candidates.flatMap((candidate) => candidate.ownedDocumentIds),
          ),
        ].sort();
        if (documentIds.length > 0) {
          const deletedOwnerships = database
            .prepare(
              `
            ...
            .run(projectId, ...blockIds, ...documentIds);
          if (deletedOwnerships.changes !== documentIds.length) {
            throw new Error("GC ownership topology changed during collection");
          }
        }
        options.faultInjector?.("after_ownership_delete");
        const deletedBlocks = database
          ...
          .run(projectId, ...blockIds);
        if (deletedBlocks.changes !== blockIds.length) {
          throw new Error("GC Block closure changed during collection");
        }
        options.faultInjector?.("after_block_delete");
        if (documentIds.length > 0) {
          const deletedDocuments = database
            .prepare(
              `DELETE FROM documents
               WHERE project_id = ? AND id IN (${placeholders(documentIds.length)})`,
            )
            .run(projectId, ...documentIds);
          if (deletedDocuments.changes !== documentIds.length) {
            throw new Error("GC Document closure changed during collection");
          }
        }
        options.faultInjector?.("after_document_delete");
        options.faultInjector?.("before_commit");
        return {
          status: "collected",
          projectId,
          collectedRootBlockIds: [...requested].sort(),
    ...
    const code =
      ...
      typeof error.code === "string"
        ? error.code
        : "";
    if (lastPlan && code.startsWith("SQLITE_CONSTRAINT")) {
      return { status: "retained", plan: appendConstraintBlocker(lastPlan) };
    }
    return {
      status: "failed",
      code: "transaction_failed",
      message: "Block retention GC transaction failed and was rolled back",
    };
  }
};
