...
import {
  PageReadStoreError,
  rebuildPageReadModelProjection,
...
const readBoardPage = async (projectId: string, pageId: string) => {
...
describe("authoritative Page reads", () => {
  ...
  sqliteTest(
    "assembles every full Page API from Block, Document, and relational metadata authorities",
    async () => {
      closeDatabase();
      const tempDir = fs.mkdtempSync(
      ...
      `,
        ...
        const byId = await getDatabaseRowPage(project.id, created.id);
        ...
        database.transaction(() => {
          rebuildPageReadModelProjection(database, project.id, [created.id]);
        })();
      ...
      `,
        ...
        closeDatabase();
        await initializeDatabase();
        const restarted = await getDatabaseRowPage(project.id, created.id);
  ...
  sqliteTest(
    "fails closed instead of serving a stale materialization or Page projection",
    async () => {
      closeDatabase();
      const tempDir = fs.mkdtempSync(
      ...
      `,
        ...
        try {
          await getDatabaseRowPage(project.id, created.id);
        } catch (error) {
          errorCode =
            error instanceof PageReadStoreError ? error.code : "unexpected";
        }
        expect(errorCode).toBe("page_materialization_stale");

        const staleProjection = database
          .prepare(
            `
        SELECT title FROM page_read_model WHERE page_block_id = ?
      `,
          )
      ...
      `,
          ...
          .run(descriptor.documentId);
        const recovered = await getDatabaseRowPage(project.id, created.id);
        expect(recovered?.title).toBe("Fresh title");

        closeDatabase();
        await initializeDatabase();
        expect((await getDatabaseRowPage(project.id, created.id))?.description).toBe(
          "Fresh body",
        );
      } finally {
        closeDatabase();
