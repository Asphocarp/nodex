...
function requireSingleNamespace(
...
export function formatDynamicToolCatalogMetrics(
  metrics: DynamicToolCatalogMetrics,
): string {
  const rows = [
    ["scope", "loading", "description", "schema", "total", "bytes/4"],
    ["namespace", "shared", "-", "-", metrics.namespaceBytes, Math.ceil(metrics.namespaceBytes / 4)],
    ["eager catalog", "eager", "-", "-", metrics.eagerBytes, Math.ceil(metrics.eagerBytes / 4)],
    ["complete catalog", "all", "-", "-", metrics.completeBytes, Math.ceil(metrics.completeBytes / 4)],
  ...
  return rows.map((row, rowIndex) => {
    const rendered = row.map((cell, column) => {
