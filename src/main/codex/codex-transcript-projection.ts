...
export function buildTranscriptFromBootstrapEvents(input: {
  ...
  const seededEntries = input.transcript
    ...
    : (input.items ?? []).map((item, index) =>
        projectCodexItemViewToTranscriptEntry(item, input.source, index),
      );
...
function isUserTranscriptEntry(entry: CodexTranscriptEntry): boolean {
  return (
    entry.role === "user" || entry.kind === "userMessage" || entry.semanticKind === "userMessage"
  );
}
...
export function projectItemToLiveTranscriptEntry(
  ...
  const existingEntry = existingTranscript.find(
    (entry) =>
  ...
  return projectCodexItemViewToTranscriptEntry(
    item,
    source,
    existingEntry?.sequence ??
      (canonicalSequence >= 0 ? canonicalSequence : existingTranscript.length),
  );
}
