...
describe("CodexConversationImageAssetService", () => {
  test("resolves a pointer through authenticated metadata and main-process bytes", async () => {
    const requestChatGptDesktop = vi.fn().mockResolvedValue(new Response(JSON.stringify({
      status: "success",
    ...
    const fetchImpl = vi.fn().mockResolvedValue(new Response("image-bytes", {
      status: 200,
    ...
    expect(requestChatGptDesktop).toHaveBeenCalledWith(expect.objectContaining({
      method: "GET",
  ...
  test("preserves explicit HTTP status and fails closed for unsupported hosts", async () => {
    const requestChatGptDesktop = vi.fn().mockResolvedValue(new Response("expired", {
      status: 425,
    }));
