...
describe("Nodex agent dynamic-tool runtime", () => {
  ...
  test("returns machine-readable stale-catalog and argument failures", async () => {
    ...
    const staleFailure = JSON.parse(
      stale.contentItems[0]?.type === "inputText" ? stale.contentItems[0].text : "null",
    ...
    const retiredFailure = JSON.parse(
      retired.contentItems[0]?.type === "inputText" ? retired.contentItems[0].text : "null",
    ) as { error?: { code?: string; recovery?: string } };
    ...
    const invalidFailure = JSON.parse(
      invalid.contentItems[0]?.type === "inputText" ? invalid.contentItems[0].text : "null",
