...
describe("cross-host thread handoff Git transaction", () => {
  test("bundles the complete dirty source tree and imports a detached managed worktree", async () => {
    ...
    await expect(readFile(path.join(imported.destinationWorkspaceRoot, "tracked.txt"), "utf8"))
      .resolves.toBe("dirty\n");
    await expect(readFile(path.join(imported.destinationWorkspaceRoot, "staged.txt"), "utf8"))
      .resolves.toBe("staged\n");
    expect(await readFile(path.join(imported.destinationWorkspaceRoot, "untracked.bin")))
      .toEqual(Buffer.from([0, 1, 2, 255]));
    await expect(readFile(imported.destinationRolloutPath, "utf8"))
      .resolves.toBe('{"type":"session_meta"}\n');
    ...
    await expect(git(destination, "rev-parse", "--verify", imported.temporaryRef)).rejects.toThrow();
    await expect(readFile(imported.destinationRolloutPath)).rejects.toMatchObject({ code: "ENOENT" });
  });
  ...
  test("fails closed when no configured destination repository has the same identity", async () => {
    ...
    await expect(importCrossHostThreadHandoff({
      requestId: "import",
      hostId: "destination",
      transferId,
