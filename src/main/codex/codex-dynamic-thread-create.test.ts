...
describe("Codex dynamic create-thread contract", () => {
  test("preserves accepted scalar whitespace and validates every exact target branch", () => {
    expect(JSON.stringify(parseCodexDynamicCreateThreadInput({
  ...
  test("rejects empty scalars, invalid environments, starting states, and efforts", () => {
    expect(parseCodexDynamicCreateThreadInput({ prompt: "", target: { type: "projectless" } })).toBe(null);
    expect(parseCodexDynamicCreateThreadInput({
      prompt: "x",
  ...
  test("projects exact model-only and thinking-only launch settings", () => {
    expect(JSON.stringify(projectCodexDynamicCreateModel("gpt-exact", undefined))).toBe(JSON.stringify({
      collaborationMode: {
        mode: "default",
        settings: {
          model: "gpt-exact",
    ...
    expect(JSON.stringify(projectCodexDynamicCreateModel(undefined, "max"))).toBe(JSON.stringify({
