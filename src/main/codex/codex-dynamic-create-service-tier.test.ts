...
function createDependencies(
  overrides: Partial<CodexCreateThreadServiceTierDependencies> = {},
...
describe("Codex dynamic create-thread service-tier resolution", () => {
  test("uses the destination host-global configured service tier", async () => {
    const calls: unknown[] = [];
    const resolved = await resolveCodexCreateThreadServiceTier(
      destination,
      createDependencies({
  ...
  test("resolves an unconfigured tier from requested, configured, then default model selection", async () => {
    ...
    expect(
      await resolveCodexCreateThreadServiceTier(
        {
          ...destination,
          model: "requested-model",
        },
  ...
  test("matches model ids and rejects catalog defaults that do not resolve to an advertised tier", async () => {
    ...
    const unavailable = await resolveCodexCreateThreadServiceTier(
      destination,
      createDependencies({
        listModels: async () => ({
          data: [
  ...
  test("retains exact from-config reads behind a disabled gate and fails closed on reads", async () => {
    const calls: string[] = [];
    const gated = await resolveCodexCreateThreadServiceTier(
      destination,
      createDependencies({
        readRequirements: async () => ({
          requirements: { featureRequirements: { fast_mode: false } },
    ...
    const phases: string[] = [];
    const configFailure = await resolveCodexCreateThreadServiceTier(
      destination,
      createDependencies({
        readConfig: async () => {
    ...
    const modelFailure = await resolveCodexCreateThreadServiceTier(
      destination,
      createDependencies({
        listModels: async () => {
    ...
    const requirementsFailure = await resolveCodexCreateThreadServiceTier(
      destination,
      createDependencies({
        readRequirements: async () => {
