...
export function computeBrowserAnnotationEvidenceCrop(
  ...
  const union = input.anchors.reduce(
    (bounds, anchor) => ({
  ...
  const right = Math.min(input.imageSize.width, Math.ceil(union.right * scaleX + padding));
  const bottom = Math.min(input.imageSize.height, Math.ceil(union.bottom * scaleY + padding));
