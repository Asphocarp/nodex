...
const BrowserPageStoreFileSchema = z.object({
  schemaVersion: z.literal(1),
...
export class FileBrowserPageSnapshotStore
implements BrowserPageSnapshotStore {
  ...
  private enforceLimits(): void {
    const ordered = [...this.pages.values()]
    ...
    while (
      this.pages.size > 0
      && encodedStoreBytes(this.pages) > MAX_STORE_BYTES
    ) {
      const oldest = [...this.pages.values()]
...
function normalizePage(
  ...
  const currentIndex = Math.max(
    ...
    Math.min(
      entries.length - 1,
      page.navigation.currentIndex - droppedCount,
    ),
  );
...
function encodedStoreBytes(
  pages: ReadonlyMap<string, BrowserSerializedPage>,
): number {
