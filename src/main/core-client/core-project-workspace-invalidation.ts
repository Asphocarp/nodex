...
export function planCoreWorkspaceNotifications(
  ...
  const project = event.projectCatalogChange
    ? {
        ...
        ...(event.projectIds.length === 1 ? { projectId: event.projectIds[0] } : {}),
      }
    : undefined;
  const sessions =
    event.sessionSummaryScopes.length > 0 || event.sessionDetailIds.length > 0
      ? {
      ...
      : undefined;
  const invalidateStandaloneRoots =
    event.projectCatalogChange === "created" || event.projectCatalogChange === "lifecycle_updated";
