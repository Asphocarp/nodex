...
export interface ProjectLifecycleCommandsOptions {
...
export const live = (
  options: ProjectLifecycleCommandsOptions,
): Layer.Layer<
  ProjectLifecycleCommands,
  never,
  BrowserSidebarRuntime | ProjectArchiveBlockers | ProjectRuntimeLifecycleRuntime | TerminalSessions
> =>
  Layer.effect(
    ProjectLifecycleCommands,
    Effect.gen(function* () {
      ...
      const project = <A>(
        ...
        Effect.tryPromise({
          try: run,
          catch: (cause) => new ProjectLifecycleCommandsError({ operation, cause }),
        });
      ...
      const readOwnership = Effect.fn("ProjectLifecycleCommands.readOwnership")(function* (
        ...
        do {
          const window = yield* project("read-ownership", () =>
            options.projectWorkspace.listProjectSessionSummaryWindow(ownedProject.id, {
      ...
      const archive = Effect.fn("ProjectLifecycleCommands.archive")(function* (
        ...
        const updated = yield* project("commit", () =>
          options.projectWorkspace.setProjectLifecycle(ownedProject.id, "archived"),
        );
      ...
      const setLifecycle = Effect.fn("ProjectLifecycleCommands.setLifecycle")(function* (
        ...
        return yield* lifecycleRuntime.runExclusive(
          projectId,
          Effect.gen(function* () {
            const ownedProject = yield* project("read-project", () =>
            ...
            const updated = yield* project("commit", () =>
              options.projectWorkspace.setProjectLifecycle(ownedProject.id, "active"),
            );
            return updated
              ? ({
                  kind: "updated",
