...
export class BrowserUseNativePipeFrameDecoder {
  ...
  constructor(private readonly maxFrameBytes = BROWSER_USE_NATIVE_PIPE_MAX_FRAME_BYTES) {}

  push(chunk: Buffer | Uint8Array): string[] {
    if (chunk.byteLength === 0) return [];
    const nextChunk = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
    this.pending =
      this.pending.byteLength === 0 ? nextChunk : Buffer.concat([this.pending, nextChunk]);
    ...
    while (this.pending.byteLength - offset >= FRAME_HEADER_BYTES) {
      ...
      messages.push(
        this.pending.subarray(offset + FRAME_HEADER_BYTES, offset + frameBytes).toString("utf8"),
      );
      offset += frameBytes;
    }
