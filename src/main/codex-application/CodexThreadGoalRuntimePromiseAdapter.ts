...
import {
  ...
  type CodexThreadGoalRuntime,
  type CodexThreadGoalSetCommand,
...
const unwrapOperationError = (error: unknown): never => {
  if (error instanceof CodexThreadGoalOperationError) throw error.cause;
  throw error;
};
...
export const makeCodexThreadGoalRuntimePromiseAdapter = (
  ...
  set: (input) => callbacks.runPromise(runtime.set(input)).catch(unwrapOperationError),
