...
export interface CodexUserInputAutoResolutionOptions {
  readonly isConversationPresented: (conversationId: string) => boolean;
}
...
export const make = (
  options: CodexUserInputAutoResolutionOptions,
): Effect.Effect<CodexUserInputAutoResolution["Service"], never, Scope.Scope> =>
  Effect.gen(function* () {
    ...
    const foregroundTimer = (tracked: TrackedUserInput) =>
      Effect.sleep(USER_INPUT_FOREGROUND_INACTIVITY).pipe(
        ...
        Effect.flatMap((continueCountdown) =>
          continueCountdown ? countdown(tracked) : Effect.void,
        ),
      );
    ...
    const schedule = (
      ...
      Effect.gen(function* () {
        ...
        yield* Ref.update(state, (entries) =>
          HashMap.set(entries, next.entry.conversationId, next),
        );
      ...
      observeRequest: (conversationId, requestId, resolve) =>
        mutations.withPermits(1)(
          Effect.gen(function* () {
            ...
            yield* schedule(
              tracked,
              options.isConversationPresented(conversationId) ? "foreground" : "background",
              true,
            );
          }),
        ),
      ...
      reevaluatePresentation: (conversationId) =>
        mutations.withPermits(1)(
          Effect.gen(function* () {
            ...
            if (options.isConversationPresented(conversationId)) {
      ...
      recordActivity: (conversationId) =>
        mutations.withPermits(1)(
          Effect.gen(function* () {
            ...
            if (!options.isConversationPresented(conversationId)) return;
