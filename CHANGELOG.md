...
- Fixed Diff-stage review search parity so `Find in review` no longer renders as a permanently visible local field; it now opens only after `Cmd/Ctrl+F` when the Diffs stage is focused, matching Codex Electron’s stage-level review find behavior.
...
- Fixed the active Threads above-composer portal styling to match Codex Electron more closely: the tasks surface now keeps the open-bottom portal shell instead of a rounded local card, and the `files changed` `+/-` summary stats now use the correct green/red semantic colors in Electron-themed threads and Storybook.
...
- Fixed the active Threads above-composer `files changed` portal to match Codex Electron’s in-progress diff surface: the portal host now uses the same `px-5` inset, and active turn diffs render as the compact summary-only `Review changes` banner instead of the completed inline file-row card.
...
### Changed
- Fixed running-thread steer prompts so they now match Codex Electron's pending-steer flow: accepted steers stay in the composer shell until the authoritative user-message item arrives, instead of inserting an early optimistic user bubble at the wrong transcript position.
