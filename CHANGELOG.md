...
- Added NFM thread mentions so card descriptions can reference and open Codex threads with `<mention-thread uuid="..." />`, with the `@` picker using command-palette chat/card search for sidebar-wide chat mentions, content snippets, fuzzy card metadata, and card description hits.
