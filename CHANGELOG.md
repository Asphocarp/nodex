...
- Added native desktop `Copy image` handling for NFM image blocks, so the toolbar now writes real image content to the clipboard and shows an in-editor error toast when native image copy fails instead of downgrading to a copied URL.
