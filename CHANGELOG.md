...
- Added a real Codex-style Diff stage review panel, so the old files-stage placeholder is replaced by a review workspace with `Last turn`, `Branch`, `Staged`, and `Unstaged` sources, a file tree, unified/split diff options, line wrapping, word diffs, review search, lazy full-file loading, rich preview toggles, Git repository initialization, refresh/copy-git-apply controls, capped large-diff handling, and file-level `Stage` / `Unstage` / `Revert` actions for workspace reviews.
...
- Shared text and number fields now use a thinner Codex-style input contract, so settings and inline form controls no longer inherit the older shadcn shadow/ring/file-input styling.
...
- Mounted Thread transcript expand/collapse behavior now uses the same Motion-style subtype-owned animation model as Codex: command, exploration, patch, MCP, reasoning, todo, answered request-user-input, and plan-summary rows animate through subtype-owned Motion wrappers, while agent-work collapse uses a separate presence animation.
- Mounted Thread transcripts no longer render a generic fallback tool row for unknown raw tool payloads; only the Codex tool families with dedicated transcript surfaces stay visible.
- The renderer theme stack now matches Codex more closely: authored CSS declares the `--vscode-*` contract, startup injects the same runtime semantic theme variables Codex derives from the active light/dark variant, and Storybook now emulates the Electron window theme classes that those contracts expect.
...
- Rebuilt the Threads stage renderer around turn-based view models and a virtualized turn list, moved unresolved approvals, request-user-input prompts, implement-plan prompts, queued follow-ups, pending steers, background terminal rows, and background child-thread activity into one Codex-style composer shell, and now derive blocked-turn state from the same turn item pipeline used for transcript rendering.
...
- The active Threads surface now matches the Codex mounted-thread fixtures more closely: previous-messages sections use the same collapse semantics, command tool rows follow the same summary/body expansion states, and expanded agent-work sections now use the same `Final message` / `Worked for …` divider behavior as Codex.
- The active Threads footer now uses one Codex-style composer shell: in-progress todo/diff cards stay in the fixed portal above the composer, while queued steers, queued follow-ups, background terminals, background child-agent rows, and live request cards are ordered by one shared shell instead of split footer surfaces; the queue lane itself now uses the narrower Codex-style pending-steer and queued-message rows with sortable drag handles, Codex-shaped action chrome, and shared stack spacing above the composer.
- Workbench settings now use a Codex-style section shell with a path-driven left rail, so the overlay keeps one active page at a time (`/settings/:section` over `General`, `Appearance`, `Editor`, `Card`, `Worktrees`, `Local environments`, `Backups`), redirects invalid section links back to the default page, and uses the same rail chrome/tokens as the upstream Codex settings UI.
- The active Threads body now uses Codex’s turn-bottom live-state model: pending turns show `Thinking` or `Exploring` inside the transcript body instead of Nodex’s old `Waiting for response…` / `Working` UI, completed reasoning stays visible without a user setting, and the legacy background `working` queue card is gone.
- Running-thread Threads composer follow-ups now follow Codex’s real `queue`/`steer` split: queueing only mutates queued-message state, queued rows no longer start a turn immediately, empty running drafts stay on `Stop`, and queued rows can be turned back into live follow-ups through the composer shell instead of a fake queue-start RPC.
- Thread request cards now follow Codex’s request family more closely: approval prompts render through the ask-for-permission shell with inline command or patch preview and `Skip`/`Submit`, background child approvals inject the worker identity inline into the prompt instead of rendering a separate header, implement-plan and request-user-input prompts share the same questionnaire shell, and MCP elicitation uses its own dedicated approval card instead of a local Nodex wrapper.
...
- Fixed the Diff-stage review options dropdown so it now follows Codex’s actions-menu contract: Git-backed review menus include `Refresh`, use the same leading action icons for diff mode, wrap, expand/collapse, full-file loading, rich preview, word diffs, and copy-git-apply, and no longer show Nodex-only right-side state text or checkmark rows.
...
- Fixed Diff-stage review panel virtualization parity so the file tree now uses a Codex-style virtualized sticky window shell, selected files stay scrolled into view inside that virtualized tree, and review diff rows opt into the same `content-visibility` rendering contract instead of rendering every tree row eagerly.
- Fixed the Codex-style settings typography scale so settings page titles like `Environments` now inherit the same 23px/28px heading variables as Codex instead of falling back to the app-wide 18px electron heading override.
- Fixed the Codex-style settings shell colors so the left rail now uses the sidebar surface token while the active settings page uses the main-surface token, matching Codex instead of swapping the two layers.
...
- Fixed Diff-stage review search parity so `Find in review` no longer renders as a permanently visible local field; it now opens only after `Cmd/Ctrl+F` when the Diffs stage is focused, matching Codex’s stage-level review find behavior.
- Fixed the Threads composer `Running terminal(s)` panel so it now follows Codex's real background-terminal model: rows come from older still-running command executions in the current conversation, the panel shows compact mono command rows with preview tooltips, and stopping background terminals now cleans that conversation-scoped running-terminal set instead of treating child-thread queue state as fake terminal activity.
- Fixed running-thread composer keyboard parity so `Cmd/Ctrl+Enter` now queues multiline follow-ups when `Cmd/Ctrl+Enter to send long prompts` is enabled, matching Codex instead of incorrectly steering as the alternate action.
- Fixed the active Threads above-composer portal styling to match Codex more closely: the tasks surface now keeps the open-bottom portal shell instead of a rounded local card, and the `files changed` `+/-` summary stats now use the correct green/red semantic colors in Electron-themed threads and Storybook.
- Fixed the active Threads above-composer shell so both above-composer hosts now render before the composer shell: fixed transcript surfaces stay in `above-composer-portal`, and queued steers, queued follow-ups, background terminals, and background agents now portal through `above-composer-queue-portal` instead of dropping below the composer.
- Fixed the active Threads above-composer `files changed` portal to match Codex’s in-progress diff surface: the portal host now uses the same `px-5` inset, and active turn diffs render as the compact summary-only `Review changes` banner instead of the completed inline file-row card.
...
- Fixed Thread-stage `turn.diff` rendering so active turns now show Codex’s compact `files changed` banner above the composer, while completed turns render a dedicated files-changed card with collapsed per-file embedded diff rows instead of a generic full-card diff viewer.
- Fixed Thread-stage message-action and MCP raw-output icon buttons in Electron so they now keep Codex's compact `icon-2xs` shipped sizing instead of expanding child SVGs to `icon-sm`.
- Fixed Thread-stage tool-call collapse ownership so command, patch, and MCP cards now follow Codex's subtype-local expand/collapse model instead of sharing a thread-level collapsed-tool map; running command rows keep their local staged shell state, patch rows toggle per file, and MCP rows no longer inherit global tool-collapse state.
...
- Fixed the mounted Thread-stage composer button to follow Codex’s running-thread state machine: an empty running draft now shows `Stop`, while any running draft switches the primary action to `Steer` or `Queue` with the same shortcut-driven alternate follow-up behavior and tooltip wording.
- Fixed Thread-stage user-message actions so `Edit message` and `Fork from here` no longer appear under later steer bubbles in the same turn; those actions now only show on the rollback/fork-eligible leading user-message prefix, matching Codex.
...
- Fixed Codex thread transcript parity for `reasoning`, `todo-list`, and answered `user-input-response` rows so the mounted thread now follows Codex’s dedicated summary/expand patterns instead of rendering those items through generic app-owned toggles.
...
- Fixed the mounted Thread-stage chrome so tool, exploration, and system rows no longer sit inside extra app-owned shell cards; the active thread body now uses the flatter Codex section layout instead of nested container-on-container styling.
- Fixed mounted Thread-stage expand/collapse jitter so ordinary rerenders no longer restart height animations; expanded thread sections now stay at natural height until the actual open/close state changes, matching Codex more closely.
...
- Fixed `Edit message` in the Thread stage to follow Codex’s inline flow: clicking `Edit` now opens an in-place edit prompt at the original user bubble, while rollback and resend only happen after clicking `Send`.
