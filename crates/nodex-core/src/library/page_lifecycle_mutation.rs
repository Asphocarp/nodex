...
fn create_page(
    ...
    let genesis_update_id = format!("page-lifecycle-genesis:{}", sha256(operation_id.as_bytes()));
...
fn restore_page(
    ...
    if let Some(requested) = requested_membership {
        ...
        if let Some(position) = &requested.position {
            let rank_key = restore_view_position(connection, page_id, requested, position, &now)?;
...
fn read_delete_receipt(
    ...
    let committed =
        serde_json::from_str::<CommittedModuleValue<LibraryCommitValue, LibraryReceipt>>(
            &result_json,
        )
...
fn restore_view_position(
    ...
    let group_key = workflow_status_key(membership.status);
    let items = connection
        .prepare(
            "SELECT page_block_id, rank_key FROM database_view_page_positions \
             WHERE view_id = ?1 AND group_key = ?2 ORDER BY rank_key, page_block_id",
        )?
        .query_map(params![position.view_id, group_key], |row| {
            Ok(RankedItem {
                id: row.get(0)?,
                rank_key: row.get(1)?,
            })
    ...
    connection.execute(
        ...
        params![position.view_id, page_id, group_key, plan.rank_key, now],
    )?;
    Ok(plan.rank_key)
}
