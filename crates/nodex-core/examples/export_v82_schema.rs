...
fn export_schema(database_path: &Path) -> Result<String, Box<dyn std::error::Error>> {
    ...
    if user_version != 82 {
        return Err(format!("schema export requires v82, received v{user_version}").into());
    }
    ...
    output.push_str("PRAGMA user_version = 82;\nCOMMIT;\nPRAGMA foreign_keys = ON;\n");
    Ok(output)
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let [_, database_path, output_path, rest @ ..] = args.as_slice() else {
        return Err("usage: export_v82_schema <v82-database> <output-sql> [--verify]".into());
    };
    ...
    if verify {
        ...
        if checked_in != schema {
            return Err(format!(
                "{} differs from the deterministic v82 schema export",
                output_path.display()
            )
            .into());
        }
